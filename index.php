...
include_once "./include/global_cookie.php";
$page=new Template();
...
if(isset($_POST['login'])) {
	...
	if($num==1) {
		...
		if ($statusUser){
			...
			$_COOKIE['Access_Page']=$accessright;*/
			setcookie('User_ID', $User_ID, time() + (86400 * 30), "/"); // 86400 = 1 day
			setcookie('Access_Page', serialize($accessright), time() + (86400 * 30), "/"); // 86400 = 1 day
			...
			if (empty($_COOKIE['Referer'])) {
				...
				echo "<meta http-equiv='refresh' content='0; url=$_COOKIE[Referer]'>";
			}
		}
		else {
			$Warning ="
