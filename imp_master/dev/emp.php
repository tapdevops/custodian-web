...
while ($db->nextRecord()) {
	...
	$m_golongan_select = "SELECT * FROM M_Grade WHERE Grade_Name = '{$db->Record['Golongan']}'";
	$db_exp->query($m_golongan_select);
	$db_exp->singleRecord();
	$emp_golID = $db_exp->Record['Grade_ID'];
	...
	// UPDATE DATA YANG TELAH ADA DI M_EMPLOYEE
		...
		$m_emp_query = "
			...
			Employee_EmailAddress = 'nicholas.budihardja@tap-agri.com',
