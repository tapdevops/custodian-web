...
function mail_release_doc($relCode,$reminder=0){
	...
	$e_query="	SELECT  User_ID,User_FullName,User_Email,DocumentGroup_Name,A_TransactionCode,
						...
						CASE WHEN THLOONLD_DocumentType = 'ORIGINAL'
							THEN 'Asli'
							ELSE CONCAT(UCASE(LEFT(THLOONLD_DocumentType, 1)),
								LCASE(SUBSTRING(THLOONLD_DocumentType, 2)))
						END AS Tipe_Dokumen,
						CASE THLOONLD_DocumentType
							WHEN 'HARDCOPY' THEN
								CASE THLOONLD_DocumentWithWatermarkOrNot
							...
							ELSE ''
						END AS Dengan_Cap,
						THLOONLD_Information AS Tujuan
				FROM TH_ReleaseOfOtherNonLegalDocuments
				INNER JOIN TH_LoanOfOtherNonLegalDocuments
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT Company_Name, TDLOONLD_DocCode,
                        ...
                        Department_Name
					FROM TH_ReleaseOfOtherNonLegalDocuments
					INNER JOIN TD_ReleaseOfOtherNonLegalDocuments
						ON TDROONLD_THROONLD_ID=THROONLD_ID
					INNER JOIN TH_LoanOfOtherNonLegalDocuments
						ON THLOONLD_LoanCode=THROONLD_THLOONLD_Code
					INNER JOIN TD_LoanOfOtherNonLegalDocuments
						ON TDROONLD_TDLOONLD_ID=TDLOONLD_ID
					INNER JOIN M_DocumentsOtherNonLegal
		...
		$bodyFooter .= '
				...
				<p align=center>
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoconl.php?cfm='.$decrp->encrypt('accept').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Setuju</a>
					</span>
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoconl.php?act='.$decrp->encrypt('reject').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Tolak</a>
					</span><br />
				</p>
				</div>
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT Company_Name,
					...
					LEFT JOIN db_master.M_Department m_d
        				ON m_d.Department_Code=DONL_Dept_Code
					WHERE THROONLD_ReleaseCode='$relCode'
					AND THROONLD_Delete_Time IS NULL";
		$ed_handle = mysql_query($ed_query);
		$edNum=1;
	...
	if (($status=='3')&&($row->User_ID==$regUser)){
		$bodyFooter .= '
				...
				<p align=center style="margin-bottom: 7%;">
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoconl.php?act='.$decrp->encrypt('confirm').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Sudah Diterima</a>
					</span>
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoconl.php?act='.$decrp->encrypt('cancel').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Batal</a>
					</span><br />
				</p>
				</div>';
	}
