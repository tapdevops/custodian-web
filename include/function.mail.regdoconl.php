...
function mail_registration_doc($regCode,$reminder=0){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="SELECT DISTINCT CASE WHEN TDROONLD_Core_CompanyID IS NOT NULL
						...
						END AS core_company_name, TDROONLD_NoDokumen, TDROONLD_NamaDokumen,
					...
					LEFT JOIN M_Department
						ON Department_ID=TDROONLD_Dept_Code
					-- LEFT JOIN M_Company
		...
		while ($ed_arr = mysql_fetch_object($ed_handle)) {
			$tahun_dokumen = date('Y', strtotime($ed_arr->TDROONLD_TahunDokumen));
			if($tahun_dokumen == '0' || $tahun_dokumen == '1970' || $tahun_dokumen == '9999' ){
				$tahun_dokumen = '-';
			}else{
				$tahun_dokumen = date('Y', strtotime($ed_arr->TDROONLD_TahunDokumen));
			}
			...
			$body .= '
						<TR  style=" font-size: 12px; font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;">
							...
							<TD>'.$ed_arr->Department_Name.'</TD>
							<TD>'.$ed_arr->TDROONLD_NamaDokumen.'</TD>
							<TD>'.$ed_arr->TDROONLD_NoDokumen.'</TD>
							<TD>'.$tahun_dokumen.'</TD>
						</TR>';
			$edNum=$edNum+1;
...
function mail_notif_registration_doc($regCode, $User_ID, $status, $attr){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="SELECT DISTINCT TDROONLD_Core_CompanyID IS NOT NULL
							THEN TDROONLD_Core_CompanyID
							ELSE THROONLD_CompanyID
						END AS core_company_id,
						CASE WHEN TDROONLD_Core_CompanyID IS NOT NULL
						...
						END AS core_company_name, Department_Name, User_FullName,
...
<tr>
	...
	<p>
		...
		<TR align="center"  style="border: 1px solid #ffe222; padding: 10px; background-color: #c4df9b; color: #333333; font-size: 12px; font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;">
			<TD style="font-size: 13px"><strong>No.</strong></TD>
			<TD style="font-size: 13px"><strong>Perusahaan</strong></TD>
			<TD style="font-size: 13px"><strong>Departemen</strong></TD>
			<TD style="font-size: 13px"><strong>Nama Dokumen</strong></TD>
			<TD style="font-size: 13px"><strong>No. Dokumen</strong></TD>
			<TD style="font-size: 13px"><strong>Tahun Dokumen</strong></TD>
		</TR>';
