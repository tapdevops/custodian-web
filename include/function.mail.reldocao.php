...
function mail_release_doc($relCode,$reminder=0){
	...
	$e_query="	SELECT  User_ID,User_FullName,User_Email,DocumentGroup_Name,A_TransactionCode,
						...
						CASE WHEN THLOAOD_DocumentType = 'ORIGINAL'
							THEN 'Asli'
							ELSE CONCAT(UCASE(LEFT(THLOAOD_DocumentType, 1)),
								LCASE(SUBSTRING(THLOAOD_DocumentType, 2)))
						END AS Tipe_Dokumen,
						CASE THLOAOD_DocumentType
							WHEN 'HARDCOPY' THEN
								CASE THLOAOD_DocumentWithWatermarkOrNot
							...
							ELSE ''
						END AS Dengan_Cap,
						THLOAOD_Information AS Tujuan
				FROM TH_ReleaseOfAssetOwnershipDocument
				INNER JOIN TH_LoanOfAssetOwnershipDocument
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT THROAOD_Reason,THLOAOD_UserID,User_FullName,
						...
						END AS expired_stnk
					FROM TH_ReleaseOfAssetOwnershipDocument
					INNER JOIN TD_ReleaseOfAssetOwnershipDocument
						ON TDROAOD_THROAOD_ID=THROAOD_ID
					INNER JOIN TH_LoanOfAssetOwnershipDocument
						ON THLOAOD_LoanCode=THROAOD_THLOAOD_Code
					INNER JOIN TD_LoanOfAssetOwnershipDocument
						ON TDROAOD_TDLOAOD_ID=TDLOAOD_ID
					INNER JOIN M_DocumentAssetOwnership
		...
		$bodyFooter .= '
				...
				<p align=center>
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldocao.php?cfm='.$decrp->encrypt('accept').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Setuju</a>
					</span>
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldocao.php?act='.$decrp->encrypt('reject').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Tolak</a>
					</span><br />
				</p>
				</div>
...
function mail_notif_release_doc($relCode, $User_ID, $status){
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT THROAOD_Reason,
						THLOAOD_UserID,THROAOD_ID,THROAOD_Information,User_FullName,
						CASE WHEN DAO_Employee_NIK LIKE 'CO@%'
						  THEN
						  	(SELECT mc.Company_Name FROM M_Company mc WHERE mc.Company_code = REPLACE(DAO_Employee_NIK, 'CO@', ''))
						  ELSE
	...
	if (($status=='3')&&($row->User_ID==$regUser)){
		$bodyFooter .= '
				...
				<p align=center style="margin-bottom: 7%;">
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldocao.php?act='.$decrp->encrypt('confirm').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Sudah Diterima</a>
					</span>
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldocao.php?act='.$decrp->encrypt('cancel').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Batal</a>
					</span><br />
				</p>
				</div>';
	}
