...
function mail_release_doc($relCode,$reminder=0){
	...
	$e_query="	SELECT  User_ID,User_FullName,User_Email,DocumentGroup_Name,A_TransactionCode,
						...
						CASE WHEN THLOLD_DocumentType = 'ORIGINAL'
							THEN 'Asli'
							ELSE CONCAT(UCASE(LEFT(THLOLD_DocumentType, 1)),
								LCASE(SUBSTRING(THLOLD_DocumentType, 2)))
						END AS Tipe_Dokumen,
						CASE THLOLD_DocumentType
							WHEN 'HARDCOPY' THEN
								CASE THLOLD_DocumentWithWatermarkOrNot
							...
							ELSE ''
						END AS Dengan_Cap,
						THLOLD_Information AS Tujuan
				FROM TH_ReleaseOfLegalDocument
				INNER JOIN TH_LoanOfLegalDocument
	...
	//$mail->AddAttachment("images/icon_addrow.png", "icon_addrow.png");  // optional name
		...
		$ed_query="	SELECT DISTINCT Company_Name,DocumentCategory_Name,DocumentType_Name,
					...
					FROM TH_ReleaseOfLegalDocument
					INNER JOIN TD_ReleaseOfLegalDocument
						ON TDROLD_THROLD_ID=THROLD_ID
					INNER JOIN TH_LoanOfLegalDocument
						ON THLOLD_LoanCode=THROLD_THLOLD_Code
					INNER JOIN TD_LoanOfLegalDocument
						ON TDROLD_TDLOLD_ID=TDLOLD_ID
					INNER JOIN M_DocumentLegal
		...
		$bodyFooter .= '
				...
				<p align=center>
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoc.php?cfm='.$decrp->encrypt('accept').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Setuju</a>
					</span>
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoc.php?act='.$decrp->encrypt('reject').'&ati='.$decrp->encrypt($row->ARC_AID).'&rdm='.$decrp->encrypt($row->ARC_RandomCode).'">Tolak</a>
					</span><br />
				</p>
				</div>
	...
	if (($status=='3')&&($row->User_ID==$regUser)){
		$bodyFooter .= '
				...
				<p align=center style="margin-bottom: 7%;">
					<span style="border: 1px solid green;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(196, 223, 155);color: #111;float: left;margin-left: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoc.php?act='.$decrp->encrypt('confirm').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Sudah Diterima</a>
					</span>
					&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
					<span style="border: 1px solid red;padding: 5px;margin-bottom: 15px; font-size: 13px;font-family: \'lucida grande\',tahoma,verdana,arial,sans-serif;background-color: rgb(239, 100, 100);color: #111;float: right;margin-right: 15%;width: 20%;border-radius: 10px;">
						<a target="_BLANK" style="color: #111;" href="http://'.$_SERVER['HTTP_HOST'].'/act.mail.reldoc.php?act='.$decrp->encrypt('cancel').'&user='.$decrp->encrypt($regUser).'&doc='.$decrp->encrypt($docID).'&rel='.$decrp->encrypt($relCode).'">Batal</a>
					</span><br />
				</p>
				</div>';
	}
