...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master RVRA
Function 			:	
...
<div>
	<fieldset>
		<legend>MASTER RVRA</legend>
		<div id='scrollarea'>
		<table width='100%' border="0" cellpadding="1" cellspacing="1" class='data_header'>
		<thead>
			<tr>
				<th>SUB RUNNING VRA CODE</th>
				<th>SUB RUNNING VRA DESC</th>
				<th>COA CODE</th>
				<th>COA DESC</th>
			</tr>
		...
		<tbody width="100%" border="0" cellpadding="3" cellspacing="1" name="data" id="data" >
...
$(document).ready(function() {
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "rvra/upd-locked-seq-status",
...
function cek_squence(){
	$.ajax({
		type    : "post",
		url     : "rvra/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "rvra/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.SUB_RVRA_CODE);
