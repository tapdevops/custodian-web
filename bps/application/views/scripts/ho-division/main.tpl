		...
		<fieldset>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" style="width:550px;" /></td>
					<td><input type="text" name="text04[]" id="text04_" style="width:600px;" /></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
			$.ajax({
				type: 'POST',
				url: 'ho-division/delete/rowid/' + encode64(rowid),
				cache: false,
				dataType: 'json',
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.DIV_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.DIV_NAME);
