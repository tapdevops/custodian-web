...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Mapping Job Type - VRA
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MAPPING JOB TYPE - VRA</legend>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td>
						...
						<input type="text" name="text02[]" id="text02_" style='width:150px;' title="Tekan F9 Untuk Memilih."/>
					</td>
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:250px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" style='width:150px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:225px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:150px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:225px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_add").live("click", function(event) {
		...
		$("#data tr:eq(" + index + ") input[id^=text03_]").attr("readonly", "true");
		$("#data tr:eq(" + index + ") input[id^=text04_]").val("");
		$("#data tr:eq(" + index + ") input[id^=text05_]").val("");
		$("#data tr:eq(" + index + ") input[id^=text05_]").attr("readonly", "true");
		$("#data tr:eq(" + index + ") input[id^=text06_]").val("");
		$("#data tr:eq(" + index + ") input[id^=text07_]").val("");
		$("#data tr:eq(" + index + ") input[id^=text07_]").attr("readonly", "true");
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		$.ajax({
			type    : "post",
			url     : "mapping-job-type-vra/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
			...
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "mapping-job-type-vra/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "mapping-job-type-vra/delete/rowid/"+encode64(rowid),
											cache    : false,
											//dataType : 'json',
											success  : function(data) {
												if (data == "done") {
	...
	$("input[id^=text02_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/rvra/module/mappingJobTypeVra/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text04_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/vra/module/mappingJobTypeVra/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/job-type/module/mappingJobTypeVra/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "mapping-job-type-vra/upd-locked-seq-status",
	...
	$.ajax({
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "mapping-job-type-vra/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.RVRA_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.SUB_RVRA_DESCRIPTION);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.VRA_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.TYPE);
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(row.JOB_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text07_]").val(row.JOB_DESCRIPTION);
					$("#data tr:eq(" + index + ")").removeAttr("style");
