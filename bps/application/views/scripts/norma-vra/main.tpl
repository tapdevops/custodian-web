...
Versi				: 	2.0.0
Deskripsi			: 	View untuk Menampilkan Norma VRA
Function 			:	
        ...
        <fieldset>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0" id="main">
				...
				<tr>
					<td>RVRA ACCOUNT :</td>
					<td>
						<input type="hidden" name="src_rvra_code" id="src_rvra_code" value="" style="width:200px;" />
						<input type="text" name="src_rvra_desc" id="src_rvra_desc" value="" style="width:200px;" class='filter'/>
						<input type="button" name="pick_rvra" id="pick_rvra" value="...">
					</td>
				</tr>
				<tr>
					<td>PENCARIAN :</td>
		...
		<fieldset>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text12[]" id="text12_" readonly="readonly" style='width:250px' value='12'/></td>
					<td><input type="text" name="text13[]" id="text13_" readonly="readonly" style='width:100px' value='13'/></td>
					<td><input type="text" name="text14[]" id="text14_" readonly="readonly" style='width:250px' value='14'/></td>
					<td>
						<input type="text" name="text15[]" id="text15_" style='width:50px' value='15'/>
						<input type="hidden" name="text16[]" id="text16_" readonly="readonly" style='width:100px' value='16'/>
					</td>
				</tr>
			</tbody>			
			</table>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-vra/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-vra/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
								...
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-vra/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-vra/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	//PICK RUNNING VRA
	$("#pick_rvra").click(function() {
		popup("pick/rvra", "pick", 700, 400 );
    });
	$("#src_rvra_desc").live("keydown", function(event) {
		//tekan F9
        if (event.keyCode == 120) {
			//lov
			popup("pick/rvra", "pick", 700, 400 );
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-vra/upd-locked-seq-status",
			...
			if (data.return == 'locked') {
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.MIN_YEAR, 0));
						$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("integer");
						$("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.MAX_YEAR, 0));
						$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("integer");
						$("#data tr:eq(" + index + ") input[id^=text08_]").val(accounting.formatNumber(row.QTY_DAY, 0));
						$("#data tr:eq(" + index + ") input[id^=text08_]").addClass("integer");
						$("#data tr:eq(" + index + ") input[id^=text09_]").val(accounting.formatNumber(row.DAY_YEAR_VRA, 0));
						$("#data tr:eq(" + index + ") input[id^=text09_]").addClass("integer");
						$("#data tr:eq(" + index + ") input[id^=text10_]").val(row.UOM);
						$("#data tr:eq(" + index + ") input[id^=text11_]").val(row.SUB_RVRA_CODE);
						$("#data tr:eq(" + index + ") input[id^=text12_]").val(row.SUB_RVRA_DESCRIPTION);
						$("#data tr:eq(" + index + ") input[id^=text13_]").val(row.MATERIAL_CODE);
						$("#data tr:eq(" + index + ") input[id^=text14_]").val(row.MATERIAL_NAME);
						$("#data tr:eq(" + index + ") input[id^=text15_]").val(accounting.formatNumber(row.QTY_UOM, 4));
						$("#data tr:eq(" + index + ") input[id^=text15_]").addClass("four_decimal");
						$("#data tr:eq(" + index + ") input[id^=text16_]").val(accounting.formatNumber(row.PRICE, 4));
