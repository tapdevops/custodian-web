...
header("Content-Disposition: attachment;Filename=report_estate_cost_afd_".date("YmdHis").".xls");
...
<div style='font-weight:bolder; font-size:20px;'>ESTATE COST per AFD</div>
...
if($this->data['count'] > 0){
	foreach($this->data['rows'] as $idx => $row){
		...
		//total per tipe
		if( ($lastBa) && ($lastTipeTransaksi) &&($row['BA_CODE'].$row['GROUP01'].$row['AFD_CODE'] <> $lastBa.$lastTipeTransaksi.$lastAfd) && ($lastActivity <> 'HA_TM')){
			//sub total
