		...
		<fieldset>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" style='width:150px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" style='width:150px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" style='width:150px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:665px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
			$.ajax({
				type: 'POST',
				url: 'ho-company/delete/rowid/' + encode64(rowid),
				cache: false,
				dataType: 'json',
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.BA_NAME);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.CORE);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.COMPANY_CODE);
					$("#data tr:eq(" + index + ") input[id^=text06_]").val(row.COMPANY_NAME);
