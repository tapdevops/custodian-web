...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan RKT Pupuk Distribusi Biaya Gabungan
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>PUPUK - DISTRIBUSI BIAYA GABUNGAN</legend>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-gabungan/get-status-periode",
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-gabungan/save-all",
				data     : $("#form_init").serialize(),
				cache    : false,
				//dataType : 'json',
				success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-pupuk-distribusi-biaya-gabungan/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_afd/" + src_afd,'_blank');
		}
    });	
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "rkt-pupuk-distribusi-biaya-gabungan/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
