...
Revisi				:	
	DRO	19/06/2013	: 	- menambahkan pengiriman data reference role untuk validasi filtering
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>NORMA HARGA BARANG</legend>
			...
			<thead>
				<tr>
					...
					<th rowspan='2'>UOM</th>
					<th colspan='2'>BUSINESS AREA</th>
					<th rowspan='2'>AVG REGION</th>
					<th rowspan='2'>AVG TAP</th>
				</tr>
				<tr>
					<th>HARGA</th>
					<th>HARGA EDIT</th>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td>
						...
						<input type="text" name="text02[]" id="text02_" readonly="readonly" style='width:75px' value='2'/>
					</td>
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:350px' value='3'/></td>
...
$(document).ready(function() {
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-harga-barang/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-harga-barang/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-harga-barang/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-harga-barang/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_calc_all").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-harga-barang/save-all",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType : 'json',
				success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-harga-barang/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-harga-barang/upd-locked-seq-status",
			...
			if (data.return == 'locked') {
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#data tr:eq(" + index + ") input[id^=text02_]").val(row.MATERIAL_CODE);
						$("#data tr:eq(" + index + ") input[id^=text03_]").val(row.MATERIAL_NAME);
						$("#data tr:eq(" + index + ") input[id^=text03_]").attr('title', row.MATERIAL_NAME);
						$("#data tr:eq(" + index + ") input[id^=text04_]").val(row.UOM);
						$("#data tr:eq(" + index + ") input[id^=text05_]").val(accounting.formatNumber(row.HARGA_KALKULASI, 2));
						$("#data tr:eq(" + index + ") input[id^=text05_]").addClass("number");
						if (row.NORMA_HARGA) {
							$("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.NORMA_HARGA, 2));
							$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("number");
						}else{
							$("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.HARGA_KALKULASI, 2));
							$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("number");
							$("#data tr:eq(" + index + ") input[id^=text06_]").css("font-style", "italic");
							$("#data tr:eq(" + index + ") input[id^=text06_]").css("background", "#f4ec7a");
						}
						$("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.AVG_REGION, 2));
						$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("number");
						$("#data tr:eq(" + index + ") input[id^=text08_]").val(accounting.formatNumber(row.AVG_PT, 2));
						$("#data tr:eq(" + index + ") input[id^=text08_]").addClass("number");
						$("#data tr:eq(" + index + ") input[id^=text09_]").val(row.PERIOD_BUDGET);
						$("#data tr:eq(" + index + ") input[id^=text10_]").val(row.BA_CODE);
