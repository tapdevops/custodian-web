...
Versi       :   1.0.0
Deskripsi     :   View untuk Menampilkan Master VRA
Function      : 
  ...
  <fieldset>
    ...
    <thead>
      <tr>
        ...
        <th>VRA CAT CODE</th>
        <th>VRA CAT DESC</th>
        <th>VRA SUB CAT CODE</th>
        <th>VRA SUB CAT DESC</th>
        <th>TYPE</th>
        <th>UOM</th>
      </tr>
    ...
    <tbody name='data' id='data'>
      <tr style="display:none">
        <td>
          ...
          <input type="text" name="text02[]" id="text02_" readonly="readonly" style='width:110px;'/>
        </td>
        <td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:110px;'/></td>
        <td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:350px;'/></td>
        <td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:120px;'/></td>
        <td><input type="text" name="text06[]" id="text06_" readonly="readonly" style='width:185px;'/></td>
        <td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:185px;'/></td>
        <td><input type="text" name="text08[]" id="text08_" readonly="readonly" style='width:120px;'/></td>
      </tr> 
    </tbody>
    </table>
    </div>
...
function cek_squence(){
  $.ajax({
    type    : "post",
    url     : "vra/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
    ...
    success : function(data) {
      if(data==1){
          $.ajax({
            type    : "post",
            url     : "vra/check-locked-seq", //check apakah status lock sendiri apakah lock
            data    : $("#form_init").serialize(),
            cache   : false,
            dataType: "json",
            success : function(data) {
    ...
    success : function(data) {
      ...
      if (count > 0) {
        $.each(data.rows, function(key, row) {
          ...
          $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.VRA_CODE);
          $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.VRA_CAT_CODE);
          $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.VRA_CAT_DESCRIPTION);
          $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.VRA_SUB_CAT_CODE);
          $("#data tr:eq(" + index + ") input[id^=text06_]").val(row.VRA_SUB_CAT_DESCRIPTION);
          $("#data tr:eq(" + index + ") input[id^=text07_]").val(row.TYPE);
