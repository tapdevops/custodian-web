...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Mapping Group CSR - COA
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MAPPING GROUP CSR - COA</legend>
...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		$.ajax({
			type    : "post",
			url     : "mapping-group-csr-coa/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
			...
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "mapping-group-csr-coa/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "mapping-group-csr-coa/delete/rowid/"+encode64(rowid),
											cache    : false,
											//dataType : 'json',
											success  : function(data) {
												if (data == "done") {
						...
						$.ajax({
							...
							success : function(data) {
								if(data.STATUS == 'LOCKED'){ 
									...
									$.ajax({
										type     : "post",
										url      : "mapping-group-csr-coa/save",
										data     : $("#form_init").serialize(),
										cache    : false,
										//dataType : 'json',
										success  : function(data) {
	...
	$("input[id^=text02_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/group-csr/module/mappingGroupCsrCoa/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text04_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/coa/module/mappingGroupCsrCoa/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "mapping-group-csr-coa/upd-locked-seq-status",
	...
	$.ajax({
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "mapping-group-csr-coa/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "mapping-group-csr-coa/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
					...
					$("#data tr:eq(" + index + ") input[id^=text04_]").attr("readonly","readonly");
