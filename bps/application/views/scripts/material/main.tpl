...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master Material
Function 			:	
		...
		<fieldset>
			...
			<thead>
				<tr>
					...
					<th>DESKRIPSI</th>
					<th>UOM</th>
					<th>VALUATION<BR>CLASS</th>
					<th>COA</th>
					<th>DESKRIPSI COA</th>
					<th>HARGA</th>
					<th>KODE<BR>NORMA DASAR</th>
					<th>INFLASI<BR>(%)</th>
					<th>FLAG</th>
					<th>DETAIL CAT CODE</th>
					<th>DETAIL CAT DESC</th>
				</tr>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:50px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:100px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" style='width:200px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:50px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text07[]" id="text07_" style='width:50px;'/></td>
					...
					<td><input type="text" name="text10[]" id="text10_" style='width:120px;'/></td>
					<td><input type="text" name="text11[]" id="text11_" style='width:50px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text12[]" id="text12_" readonly="readonly" style='width:50px;'/></td>
					<td><input type="text" name="text13[]" id="text13_" style='width:50px;' title="Tekan F9 Untuk Memilih, Tekan del Untuk Menghapus."/></td>
					<td><input type="text" name="text14[]" id="text14_" style='width:50px;'/></td>
					<td><input type="text" name="text15[]" id="text15_" style='width:300px;'/></td>					
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "material/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					...
					if(data==1){
							$.ajax({
								type    : "post",
								url     : "material/check-locked-seq", //check apakah status lock sendiri apakah lock
								data    : $("#form_init").serialize(),
								cache   : false,
								dataType: "json",
								success : function(data) {
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			alert("Field Merah harus diisi terlebih dahulu.");
		}else {
			$.ajax({
				type    : "post",
				url     : "material/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-material/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/uom/module/material/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text08_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/coa/module/material/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text11_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/norma-basic/module/material/row/" + row + "/bacode/" + bacode, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text13_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/flag-panen-non-panen/module/material/row/" + row, "pick", 700, 400 );
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "material/upd-locked-seq-status",
...
function validate(){	
	...
	$('input[id^=text13_]').each(function(key, row) {
		if (key > 0){
			var mystring = this.value;
			if (document.getElementById("text11_" + key).value == 'NC018') {
				if(mystring == ''){
					$(this).addClass("error");
					$(this).focus();
					result = false;
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
					...
					$("#data tr:eq(" + index + ") input[id^=text07_]").val(row.VALUATION_CLASS);
                    $("#data tr:eq(" + index + ") input[id^=text08_]").val(row.COA_CODE);
					$("#data tr:eq(" + index + ") input[id^=text09_]").val(row.COA_DESC);
					...
					$("#data tr:eq(" + index + ") input[id^=text11_]").val(row.BASIC_NORMA_CODE);
					$("#data tr:eq(" + index + ") input[id^=text12_]").val(accounting.formatNumber(row.PERCENT_INCREASE, 2));
					$("#data tr:eq(" + index + ") input[id^=text12_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text13_]").val(row.FLAG);
					$("#data tr:eq(" + index + ") input[id^=text14_]").val(row.DETAIL_CAT_CODE);
					$("#data tr:eq(" + index + ") input[id^=text15_]").val(row.DETAIL_CAT_DESC);
