...
header("Content-Disposition: attachment;Filename=report_keb_aktivitas_est_cost_afd".date("YmdHis").".xls");
...
if($this->data['count'] > 0){
	...
	foreach($this->data['rows'] as $idx => $row){
		...
		/*if ($cekgroup1 == $row['BA_CODE'].$row['AFD_CODE'].$row['ESTATE_NAME'].$row['TIPE_TRANSAKSI'].$row['DESCRIPTION']){
			$BA_CODE = "";
			$AFD_CODE = "";
			$ESTATE_NAME = "";
			$TIPE_TRANSAKSI = "";
		...
		/*$cekgroup1 = $row['BA_CODE'].$row['AFD_CODE'].$row['ESTATE_NAME'].$row['TIPE_TRANSAKSI'].$row['DESCRIPTION']; 
