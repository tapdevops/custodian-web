...
header("Content-Disposition: attachment;Filename=report_kebutuhan_aktivitas_est_cost_ba".date("YmdHis").".xls");
...
<body>
<div style='font-weight:bolder; font-size:20px;'>REPORT KEBUTUHAN AKTIVITAS ".date('Y', strtotime($this->data['rows'][0]['PERIOD_BUDGET']))."</div>
...
if($this->data['count'] > 0){
	...
	foreach($this->data['rows'] as $idx => $row){
		/*if ($cekgroup1 == $row['BA_CODE'].$row['ESTATE_NAME'].$row['TIPE_TRANSAKSI'].$row['DESCRIPTION']){
			$BA_CODE = "";
			$ESTATE_NAME = "";
			$TIPE_TRANSAKSI = "";
			$DESCRIPTION = "";
		...
		echo "<td><b>".$BA_CODE."</b></td>";
		echo "<td><b>".$ESTATE_NAME."</b></td>";*/
		$curGroup = "";
		for ($i = 1 ; $i <= $this->data['max_group'] ; $i++){
		...
		/*$cekgroup1 = $row['BA_CODE'].$row['ESTATE_NAME'].$row['TIPE_TRANSAKSI'].$row['DESCRIPTION']; 
