...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan RKT Pupuk KG Sisip
Function 			:	
		...
		<fieldset>
			...
			<tbody name='data' id='data'>
				<tr name='content' id='content' style="display:none;" >
					...
					<td width='75px'><input type="text" name="text40[]" id="text40_" readonly="readonly" /></td>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-kg-sisip/get-status-periode",
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-kg-sisip/save-all",
				data     : $("#form_init").serialize(),
				cache    : false,
				//dataType : 'json',
				success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-pupuk-kg-sisip/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_afd/" + src_afd + "/src_jenis_pupuk/" + src_jenis_pupuk,'_blank');
		}
    });
        ...
        success : function(data) {
            ...
            if (count > 0) {
				$.each(data.rows, function(key, row) {
					...
					$("#data_freeze tr:eq(" + index + ") input[id^=text40_]").val(row.BLOCK_CODE + " - " + row.BLOCK_DESC);
					...
					var split_block = row.BLOCK_CODE.split(" - ");
