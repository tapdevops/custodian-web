...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Mapping Group BUM - COA
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MAPPING GROUP BUM - COA</legend>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text04[]" id="text04_" style='width:200px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:450px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		$.ajax({
			type    : "post",
			url     : "mapping-group-bum-coa/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
			...
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "mapping-group-bum-coa/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "mapping-group-bum-coa/delete/rowid/"+encode64(rowid),
											cache    : false,
											//dataType : 'json',
											success  : function(data) {
												if (data == "done") {
						...
						$.ajax({
							...
							success : function(data) {
								if(data.STATUS == 'LOCKED'){ 
									...
									$.ajax({
										type     : "post",
										url      : "mapping-group-bum-coa/save",
										data     : $("#form_init").serialize(),
										cache    : false,
										//dataType : 'json',
										success  : function(data) {
	...
	$("input[id^=text02_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/group-bum/module/mappingGroupBumCoa/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text04_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/coa/module/mappingGroupBumCoa/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "mapping-group-bum-coa/upd-locked-seq-status",
	...
	$.ajax({
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "mapping-group-bum-coa/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "mapping-group-bum-coa/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.GROUP_BUM_CODE);
