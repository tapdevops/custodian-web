		...
		<fieldset>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="50%">	
						...
						&nbsp;&nbsp;&nbsp;
						<input type="button" name="btn_template_csv" id="btn_template_csv" value="TEMPLATE DATA" class="button" />
					</td>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td width='200px'>
						<input type="text" name="text05[]" id="text05_" readonly="readonly" value='5'/>
						
						
					</td>
					
					
					<td style="display:none" ><input type="hidden" name="text25[]" id="text25_" readonly="readonly" value='25'/></td>
					...
					<td width='100px'><input type="text" name="text23[]" id="text23_" value='23'/></td>
				</tr>
			</tbody>
			</table>
			<!--/div-->
...
//FREEZE PANES
	...
	//Added by Ardo 23/08/2016 - Perubahan proses perhitungan perkerasan jalan (external)
	$("#btn_template_csv").live("click", function() {	
		//DEKLARASI VARIABEL
		var reference_role = "<?=$this->referencerole?>";	//REFERENCE_ROLE
		var budgetperiod = $("#budgetperiod").val();		//PERIODE BUDGET
		var current_budgetperiod = "<?=$this->period?>";	//PERIODE BUDGET SEBELUMNYA / TAHUN BERJALAN
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/template-report-norma-perkerasan-jalan/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						$.ajax({
							type     : "post",
							url      : "report-norma-perkerasan-jalan/save",
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType : 'json',
							success  : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
					...
					$("#data tr:eq(" + index + ") input[id^=text25_]").val(row.JARAK_RANGE);
