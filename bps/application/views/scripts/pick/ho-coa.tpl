...
$(document).ready(function() {
	$("#table1 tr input[id^=pick]").live("click", function() {
        ...
        if(module == 'hoCapex'){
            ...
            opener.$('#text14_' + row).val(decodeHtml(arr[2]));
            ...
            opener.$('#text05_' + row).val(decodeHtml(arr[2]));
            opener.addClassEdited(row);
        } else if (module == 'hoSpd') {
            opener.$('#text07_' + row).val(arr[1]);
            opener.$('#text08_' + row).val(decodeHtml(arr[2]));
            opener.addClassEdited(row);
        } else {
            opener.$('input[id^=text04_]').val(arr[1]);
