...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Norma Harga Barang
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>NORMA HARGA BORONG</legend>
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					...
					<td width="50%" align="right">		
						...
						<!--<input type="button" name="btn_calc_all" id="btn_calc_all" value="HITUNG SEMUA ITEM" class="button"/>		-->
			...
			<thead>
				<tr>
					...
					<th>SPESIFIKASI</th>
					<th>UOM</th>
					<th>UMUM<BR>HARGA EXTERNAL<BR>(RP/QTY)</th>
					<th>UMUM<BR>AVG REGION</th>
					<th>UMUM<BR>AVG TAP</th>
					<th>KHUSUS<BR>HARGA EXTERNAL<BR>(RP/QTY)</th>
					<th>KHUSUS<BR>AVG REGION</th>
					<th>KHUSUS<BR>AVG TAP</th>
				</tr>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text06[]" id="text06_" style='width:50px' value='6' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text07[]" id="text07_" style='width:200px' value='7'/></td>
					<td><input type="text" name="text08[]" id="text08_" readonly="readonly" style='width:125px' value='8'/></td>
					<td><input type="text" name="text09[]" id="text09_" style='width:125px' value='9'/></td>
					<td><input type="text" name="text10[]" id="text10_" readonly="readonly" style='width:125px' value='10'/></td>
					<td><input type="text" name="text11[]" id="text11_" readonly="readonly" style='width:125px' value='11'/></td>
					
					<td><input type="text" name="text12[]" id="text12_" style='width:125px' value='12'/></td>
					<td><input type="text" name="text13[]" id="text13_" readonly="readonly" style='width:125px' value='13'/></td>
					<td><input type="text" name="text14[]" id="text14_" readonly="readonly" style='width:125px' value='14'/></td>
				</tr>
			</tbody>			
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-harga-barang/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					...
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-harga-borong/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-harga-borong/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		$.ajax({
			type    : "post",
			url     : "norma-harga-borong/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
			data    : $("#form_init").serialize(),
			cache   : false,
			dataType: "json",
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "norma-harga-borong/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "norma-harga-borong/delete/rowid/"+encode64(rowid),
											cache    : false,
											dataType : 'json',
											success  : function(data) {
	...
	$("#btn_calc_all").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-harga-borong/save-all/region/" + src_region_code,
				cache    : false,
				//dataType : 'json',
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-harga-borong/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-harga-borong/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			if ($('#text06_'+row).is('[readonly]') == false) { 
				//lov
				popup("pick/activity-class/module/normaHargaBorong/row/" + row, "pick", 700, 400 );
			}
        }else{
			event.preventDefault();
		}
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-harga-borong/upd-locked-seq-status",
		...
		if (data.return == 'locked') {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
					...
					$("#data tr:eq(" + index + ") input[id^=text05_]").val(row.ACTIVITY_DESCRIPTION);
					$("#data tr:eq(" + index + ") input[id^=text06_]").val(row.ACTIVITY_CLASS);
					$("#data tr:eq(" + index + ") input[id^=text07_]").val(row.SPESIFICATION);
					$("#data tr:eq(" + index + ") input[id^=text08_]").val(row.UOM);
                    $("#data tr:eq(" + index + ") input[id^=text09_]").val(accounting.formatNumber(row.PRICE, 2));
					$("#data tr:eq(" + index + ") input[id^=text09_]").addClass("number");
                    $("#data tr:eq(" + index + ") input[id^=text10_]").val(accounting.formatNumber(row.AVG_REGION, 2));
					$("#data tr:eq(" + index + ") input[id^=text10_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text11_]").val(accounting.formatNumber(row.AVG_PT, 2));
					$("#data tr:eq(" + index + ") input[id^=text11_]").addClass("number");
					
					$("#data tr:eq(" + index + ") input[id^=text12_]").val(accounting.formatNumber(row.PRICE_SITE, 2));
					$("#data tr:eq(" + index + ") input[id^=text12_]").addClass("number");
                    $("#data tr:eq(" + index + ") input[id^=text13_]").val(accounting.formatNumber(row.AVG_REGION_SITE, 2));
					$("#data tr:eq(" + index + ") input[id^=text13_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text14_]").val(accounting.formatNumber(row.AVG_PT_SITE, 2));
					...
					if(user_role != "ADMIN"){ //jika bukan admin(user HO), maka norma HO akan readonly
						$("#data tr:eq(" + index + ") input[id^=text09_]").attr("readonly", "true");
					}
