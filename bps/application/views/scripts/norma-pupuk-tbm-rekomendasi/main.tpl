...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-pupuk-tbm-rekomendasi/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-pupuk-tbm-rekomendasi/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
							$("#btn_save_temp").hide();
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-pupuk-tbm-rekomendasi/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
		...
		$.ajax({
			...
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "norma-pupuk-tbm-rekomendasi/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "norma-pupuk-tbm-rekomendasi/delete/rowid/"+encode64(rowid),
											//data     : $("#form_init").serialize(),
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-pupuk-tbm-rekomendasi/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
						...
						$.ajax({
							...
							success : function(data) {
								if(data.STATUS == 'LOCKED'){ 
									...
									$.ajax({
										type     : "post",
										url      : "norma-pupuk-tbm-rekomendasi/save",
										data     : $("#form_init").serialize(),
										cache    : false,
										dataType : 'json',
										success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-pupuk-tbm-rekomendasi/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-pupuk-tbm-rekomendasi/upd-locked-seq-status",
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "norma-pupuk-tbm-rekomendasi/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
