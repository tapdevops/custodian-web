...
Versi				: 	2.0.0
Deskripsi			: 	View untuk Menampilkan Norma Panen OER BJR
Function 			:	
...
YULIUS 08/07/2014	: 	- fungsi save temp hanya dilakukan ketika ada perubahan data & pindah ke next page
						- penambahan info untuk lock table pada tombol cari, simpan
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>NORMA PANEN OER BJR</legend>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="15%"><b>Asumsi Over Basis (%) :</b></td>
					<td width="85%">
						<input type="text" name="asumsi_over_basis" id="asumsi_over_basis" value="" style="width:70px;"/>
						<input type="hidden" name="asumsi_over_basis_old" id="asumsi_over_basis_old" value="" style="width:70px;"/>
					</td>
				</tr>
			</table>
			<div id='scrollarea'>
			...
			<thead>
				<tr>
					...
					<th>JANJANG BASIS MANDOR<BR>DAN PEMANEN</th>
					<th>JANJANG BASIS MANDOR<BR>DAN PEMANEN (JUMAT)</th>
					<th>BJR BUDGET</th>
					<th>JANJANG<BR>OPERATION</th>
					<th>OVER BASIS<BR>JANJANG</th>
					<th>MIN OER</th>
					<th>MAX OER</th>
					<th>RP/JJG</th>
					<th>RP/KG</th>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text15[]" id="text15_" style='width:120px' value='15'/></td>
					...
					<td><input type="text" name="text12[]" id="text12_" style='width:100px' value='12'/></td>
					<td>
						<input type="text" name="text13[]" id="text13_" readonly="readonly" style='width:120px' value='13'/>
						<input type="hidden" name="text14[]" id="text14_" readonly="readonly" style='width:120px' value='14'/>
					</td>
				</tr>
			</tbody>
			</table>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if  ( ba_code == '' ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-panen-oer-bjr/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-panen-oer-bjr/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
							$("#btn_save_temp").hide();
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-panen-oer-bjr/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
	...
	$("#btn_save").click( function() {//DEKLARASI VARIABEL
		var reference_role = "<?=$this->referencerole?>";	//REFERENCE_ROLE
		var budgetperiod = $("#budgetperiod").val();		//PERIODE BUDGET
		var current_budgetperiod = "<?=$this->period?>";	//PERIODE BUDGET SEBELUMNYA / TAHUN BERJALAN
		var src_region_code = $("#src_region_code").val();	//KODE REGION
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-panen-oer-bjr/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-panen-oer-bjr/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-panen-oer-bjr/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-panen-oer-bjr/upd-locked-seq-status",
		...
		if (data.return == 'locked') {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
					...
					$("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.JANJANG_BASIS_MANDOR, 0));
					$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text15_]").val(accounting.formatNumber(row.JANJANG_BASIS_MANDOR_JUMAT, 0));
					$("#data tr:eq(" + index + ") input[id^=text15_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.BJR_BUDGET, 0));
					$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text08_]").val(accounting.formatNumber(row.JANJANG_OPERATION, 2));
					$("#data tr:eq(" + index + ") input[id^=text08_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text09_]").val(accounting.formatNumber(row.OVER_BASIS_JANJANG, 2));
					$("#data tr:eq(" + index + ") input[id^=text09_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text10_]").val(accounting.formatNumber(row.OER_MIN, 2));
					$("#data tr:eq(" + index + ") input[id^=text10_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text11_]").val(accounting.formatNumber(row.OER_MAX, 2));
					$("#data tr:eq(" + index + ") input[id^=text11_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text12_]").val(accounting.formatNumber(row.PREMI_PANEN, 2));
					$("#data tr:eq(" + index + ") input[id^=text12_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text13_]").val(accounting.formatNumber(row.NILAI, 2));
					$("#data tr:eq(" + index + ") input[id^=text13_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text14_]").val(accounting.formatNumber(row.OER, 2));
					...
					$("#asumsi_over_basis_old").val(accounting.formatNumber(row.ASUMSI_OVER_BASIS, 2));
					$("#asumsi_over_basis_old").addClass("number");
                });
            }
		  }	
        }
