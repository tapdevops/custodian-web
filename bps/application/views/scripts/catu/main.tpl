...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master Catu
Function 			:	
		...
		<fieldset>
			...
			<thead>
				<tr>
					...
					<th>RICE PORTION</th>
					<th>HARGA / KG</th>
					<th>HARI KERJA / BULAN</th>
					<th>CATU BERAS / BULAN</th>
				</tr>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					<td>
						...
						<input type="text" name="text02[]" id="text02_" readonly="readonly" style='width:170px;'/>
					</td>
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:170px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:170px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" style='width:170px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:170px;'/></td>
					<td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:170px;'/></td>
					<td><input type="text" name="text08[]" id="text08_" readonly="readonly" style='width:170px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "catu/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					...
					if(data==1){
							$.ajax({
								type    : "post",
								url     : "catu/check-locked-seq", //check apakah status lock sendiri apakah lock
								data    : $("#form_init").serialize(),
								cache   : false,
								dataType: "json",
								success : function(data) {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-catu/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "catu/upd-locked-seq-status",
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.EMPLOYEE_STATUS);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(accounting.formatNumber(row.RICE_PORTION, 2));
					$("#data tr:eq(" + index + ") input[id^=text05_]").addClass("number");
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.PRICE_KG, 2));
					$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("number");
                    $("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.HKE_BULAN, 0));
					$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text08_]").val(accounting.formatNumber(row.CATU_BERAS, 2));
