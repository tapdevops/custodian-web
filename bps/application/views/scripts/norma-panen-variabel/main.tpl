...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Norma Panen Variabel
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>NORMA PANEN VARIABEL</legend>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:250px' value='4'/></td>
					<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:450px' value='5'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:200px' value='6'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-panen-variabel/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-panen-variabel/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
							$("#btn_save_temp").hide();
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-panen-variabel/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			alert("Jumlah Presentase Brondolan dan Presentase Janjang Harus 100%.");
		} */else {
			$.ajax({
				type    : "post",
				url     : "norma-panen-variabel/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-panen-variabel/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-panen-variabel/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-panen-variabel/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-panen-variabel/upd-locked-seq-status",
...
//validasi presentase brondolan & presentase janjang
function validate(){	
	var result = true;
	...
	$('input[id^=text04_]').each(function(key, row) {
		if (key > 0) {
			var row = $(this).attr("id").split("_")[1];
			
			if ($("#text04_" + row).val() == 'PRES_BRONDOLAN'){
				var mystring = $("#text06_" + row).val();
				brondolan = mystring.split(',').join('');
				row_brondolan = row;
			}
			
			if ($("#text04_" + row).val() == 'PRES_JANJANG'){
				var mystring = $("#text06_" + row).val();
				janjang = mystring.split(',').join('');
				row_janjang = row;
			}
		}
	});
	
	var total = parseFloat(brondolan) + parseFloat(janjang);
	if ( parseFloat(total) != parseFloat(100) ){
		$("#text06_" + row_brondolan).addClass("error");
		$("#text06_" + row_janjang).addClass("error");
		result = false;
	}else{
		$("#text06_" + row_brondolan).removeClass("error");
		$("#text06_" + row_janjang).removeClass("error");
	}
		...
		if (data.return == 'locked') {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.PANEN_CODE);
					$("#data tr:eq(" + index + ") input[id^=text05_]").val(row.DESCRIPTION);
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.VALUE, 2));
					$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("number");
					
					if (row.PANEN_CODE == 'VRA_DT_EXTERNAL'){
						$("#data tr:eq(" + index + ") input[id^=text06_]").attr("readonly", "readonly");
					}else{
						$("#data tr:eq(" + index + ") input[id^=text06_]").removeAttr('readonly');
					}
