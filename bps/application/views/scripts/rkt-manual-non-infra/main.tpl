		...
		<fieldset>
			...
			<tbody width='100%' name='data' id='data'>
				<tr name='content' id='content' style="display:none;" >
					...
					<!-- BULAN AWAL ROTASI -->
					<td width='100px'><select name="text48[]" id="text48_"></select></td>					
					<td width='150px'><select name="text19[]" id="text19_"></select><!--<input type="text" name="text19[]" id="text19_" readonly="readonly"/>--></td>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				...
				success  : function(data) {
					...
					if (count > 0) {
						...
						$.each(data.rows, function(key, row) {
							$('#text19_').append(new Option(row.NILAI,row.NILAI));
						});						
					}else{
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-non-infra/get-tipe-norma",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType: "json",
				success  : function(data) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-non-infra/get-activity-class",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType: "json",
				success  : function(data) {
						...
						$.ajax({
							...
							success : function(data) {
								if(data==1){
									...
									$.ajax({
										type     : "post",
										url      : "rkt-manual-non-infra/get-status-periode", //cek status periode
										data     : $("#form_init").serialize(),
										cache    : false,
										dataType: "json",
										success  : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-non-infra/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( src_region_code == '' ) && ( key_find == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-manual-non-infra/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_activity_code/" + src_activity_code + "/src_afd/" + src_afd,'_blank');
		}
    });
	...
	$("#pick_activity").click(function() {
		popup("pick/activity-mapp/module/rktManualNonInfra", "pick", 700, 400 );
    });
	$("#src_activity_desc").live("keydown", function(event) {
		//tekan F9
        if (event.keyCode == 120) {
			//lov
			popup("pick/afdeling-mapp/module/rktManualNonInfra", "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "rkt-manual-non-infra/upd-locked-seq-status",
			...
			if (data.return == 'locked') {
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#text19_" + index + " option[value='"+row.SUMBER_BIAYA+"']").attr("selected", "selected");
