...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Mapping Job Type - WRA
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MAPPING JOB TYPE - WRA</legend>
			...
			<thead>								
				<tr>
					...
					<th>WRA GROUP CODE</th>
					<th>WRA GROUP DESC</th>
					<th>WRA FLAG</th>
					<th>JOB CODE</th>
					<th>JOB DESC</th>
				</tr>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text04[]" id="text04_" style='width:150px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" style='width:150px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text06[]" id="text06_" readonly="readonly" style='width:350px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_add").live("click", function(event) {
		...
		$("#data tr:eq(" + index + ") input[id^=text06_]").attr("readonly", "true");
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		$.ajax({
			type    : "post",
			url     : "mapping-job-type-wra/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
			...
			success : function(data) {
				if(data==1){
					...
					$.ajax({
						type    : "post",
						url     : "mapping-job-type-wra/check-locked-seq",
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
							if(data.STATUS == 'LOCKED'){ 
								...
								if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
									...
									if (rowid == '') {
										...
										$.ajax({
											type     : "post",
											url      : "mapping-job-type-wra/delete/rowid/"+encode64(rowid),
											cache    : false,
											//dataType : 'json',
											success  : function(data) {
												if (data == "done") {
	...
	$("input[id^=text02_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/wra-group/module/mappingJobTypeWra/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text05_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/job-type/module/mappingJobTypeWra/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "mapping-job-type-wra/upd-locked-seq-status",
	...
	$.ajax({
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "mapping-job-type-wra/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.WRA_GROUP_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.PARAMETER_VALUE);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.WRA_FLAG);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.JOB_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(row.JOB_DESCRIPTION);
					$("#data tr:eq(" + index + ")").removeAttr("style");
