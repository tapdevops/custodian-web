...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		var search = $("#search").val();					//SEARCH FREE TEXT
			...
			$.ajax({
				type    : "post",
				url     : "norma-sph/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				//		$.ajax({
				//			type     : "post",
				//			url      : "norma-sph/get-status-periode", //cek status periode
				//			data     : $("#form_init").serialize(),
				//			cache    : false,
				//			dataType: "json",
				//			success  : function(data) {
				...
				//					}else{
										$.ajax({
											type    : "post",
											url     : "norma-sph/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-sph/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	$("#btn_export_csv").live("click", function() {	
		//DEKLARASI VARIABEL
		var search = $("#search").val();					//SEARCH FREE TEXT	
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-Sph/search/" + encode64(search),'_blank');
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-sph/upd-locked-seq-status",
			...
			if (data.return == 'locked') {
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#data tr:eq(" + index + ") input[id^=text02_]").val(row.CORE);
						$("#data tr:eq(" + index + ") input[id^=text03_]").val(row.LAND_TYPE);
						$("#data tr:eq(" + index + ") input[id^=text04_]").val(row.TOPOGRAPHY);
						$("#data tr:eq(" + index + ") input[id^=text05_]").val(row.SPH_STANDAR);
