...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Mapping Activity - Coa
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MAPPING AKTIVITAS - COA</legend>
			...
			<thead>
				<tr>
					...
					<th>GRUP AKTIVITAS</th>
					<th>KODE AKTIVITAS</th>
					<th>DESKRIPSI</th>
					<th>COST ELEMENT</th>
					<th>KODE COA</th>
					<th>DESKRIPSI COA</th>
					<th>KODE AKTIVITAS<BR>DI SAP</th>
				</tr>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:150px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:200px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:150px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:100px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:400px;'/></td>
					<td><input type="text" name="text08[]" id="text08_" style='width:100px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("input[id^=btn00_]").live("click", function(event) {
		...
		$("#data tr:eq(" + index + ") input[id^=text03_]").attr("readonly", "");
		$("#data tr:eq(" + index + ") input[id^=text03_]").attr("title", "Tekan F9 Untuk Memilih.");
		...
		$("#data tr:eq(" + index + ") input[id^=text05_]").attr("title", "Tekan F9 Untuk Memilih.");
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
			...
			if (rowid == '') {
				...
				$.ajax({
					type     : "post",
					url      : "mapping-activity-coa/delete/rowid/"+encode64(rowid),
					cache    : false,
					//dataType : 'json',
	...
	$("input[id^=text02_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			if ($('#text02_'+row).is('[readonly]') == false) { 
				//lov
				popup("pick/activity-group/module/mappingActivityCoa/row/" + row, "pick", 700, 400 );
			}
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text03_]").live("keydown", function(event) {
		var row = $(this).attr("id").split("_")[1];
		
		//tekan F9
        if (event.keyCode == 120) {
			if ($('#text03_'+row).is('[readonly]') == false) { 
				//lov
				popup("pick/activity/module/mappingActivityCoa/row/" + row, "pick", 700, 400 );
			}
        }else{
			event.preventDefault();
		}
	...
	$("input[id^=text05_]").live("keydown", function(event) {
		...
		//tekan F9
        if (event.keyCode == 120) {
			if ($('#text05_'+row).is('[readonly]') == false) { 
				//lov
				popup("pick/cost-element/module/mappingActivityCoa/row/" + row, "pick", 700, 400 );
			}
        }else{
			event.preventDefault();
		}
	...
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			if ($('#text06_'+row).is('[readonly]') == false) { 
				//lov
				popup("pick/coa/module/mappingActivityCoa/row/" + row, "pick", 700, 400 );
			}
        }else{
			event.preventDefault();
		}
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "mapping-activity-coa/upd-locked-seq-status",
...
function cek_squence(){
	$.ajax({
		type    : "post",
		url     : "mapping-activity-coa/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
		data    : $("#form_init").serialize(),
		cache   : false,
		dataType: "json",
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "mapping-activity-coa/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.ACTIVITY_GROUP);
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.ACTIVITY_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.ACTIVITY_DESC);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.COST_ELEMENT);
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(row.COA_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text07_]").val(row.COA_DESC);
					$("#data tr:eq(" + index + ") input[id^=text08_]").val(row.ACTIVITY_CODE_SAP);
					$("#data tr:eq(" + index + ")").removeAttr("style");
