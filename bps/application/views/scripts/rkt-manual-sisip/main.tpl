...
Versi				: 	2.0.0
Deskripsi			: 	View untuk Menampilkan RKT Manual - Non Infra
Function 			:	
...
Revisi				:	
	...
	SID 18/07/2014	: 	- penambahan combo box untuk pilihan norma (text49)
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0" id="main">
				...
				<tr>
					...
					<td>
						...
						<input type="text" name="src_activity_desc" id="src_activity_desc" value="" style="width:200px;" class='filter src_activity'/>
			...
			<tbody width='100%' name='data' id='data'>
				<tr name='content' id='content' style="display:none;" >
					<td width='50px'>
						...
						<input type="hidden" name="tChange[]" id="tChange_" readonly="readonly" class="valPok"/>
						<input type="hidden" name="text01[]" id="text01_" readonly="readonly"/>
						<input type="hidden" name="maxNorma[]" id="maxNorma_" readonly="readonly"/>
					...
					<td width='100px'><select name="text49[]" id="text49_"> </select></td> <!-- TIPE NORMA -->
					...
					<!-- BULAN AWAL ROTASI -->				
					<td width='150px'><input type="text" name="text19[]" id="text19_" readonly="readonly"/></td>
					<td width='100px'><input type="text" name="text20[]" id="text20_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text21[]" id="text21_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text22[]" id="text22_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text23[]" id="text23_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text24[]" id="text24_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text25[]" id="text25_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text26[]" id="text26_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text27[]" id="text27_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text28[]" id="text28_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text29[]" id="text29_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text30[]" id="text30_" class='integer val_dis'/></td>
					<td width='100px'><input type="text" name="text31[]" id="text31_" class='integer val_dis'/></td>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-sisip/get-tipe-norma",
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-sisip/get-activity-class",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType: "json",
				success  : function(data) {
					...
					if (count > 0) {
						...
						$.ajax({
							type    : "post",
							url     : "rkt-manual-sisip/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
								if(data==1){
									...
									$.ajax({
										type     : "post",
										url      : "rkt-manual-sisip/get-status-periode", //cek status periode
										data     : $("#form_init").serialize(),
										cache    : false,
										dataType: "json",
										success  : function(data) {
											...
											if (data == 'CLOSE') {
													...
													/*$.ajax({
														type    : "post",
														url     : "rkt-manual-non-infra/check-locked-seq", //check apakah status lock sendiri apakah lock
														data    : $("#form_init").serialize(),
														cache   : false,
														dataType: "json",
														success : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-manual-sisip/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						/*$.ajax({
							type    : "post",
							url     : "rkt-manual-non-infra/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
								...
								//validasi jumlah pokok sebelum proses hitung
								statHit = 0;
								$('.valPok').each(function(index, elem) {
									valu = $(elem).val();
									if (valu != ""){
										rid = $(elem).attr('id').split("_")[1];
										valmonth = Number($("#text20_"+rid).val().replace(",", ""))+
													Number($("#text21_"+rid).val().replace(",", ""))+
													Number($("#text22_"+rid).val().replace(",", ""))+
													Number($("#text23_"+rid).val().replace(",", ""))+
													Number($("#text24_"+rid).val().replace(",", ""))+
													Number($("#text25_"+rid).val().replace(",", ""))+
													Number($("#text26_"+rid).val().replace(",", ""))+
													Number($("#text27_"+rid).val().replace(",", ""))+
													Number($("#text28_"+rid).val().replace(",", ""))+
													Number($("#text29_"+rid).val().replace(",", ""))+
													Number($("#text30_"+rid).val().replace(",", ""))+
													Number($("#text31_"+rid).val().replace(",", ""));
										maxPok = $("#maxNorma_"+rid).val();		
										if (valmonth > maxPok){			
											$("#data_freeze tr:eq(" + rid + ") select[id^=text], #data_freeze tr:eq(" + rid + ") input[id^=text]").addClass("edited");
											$("#data tr:eq(" + rid + ") select[id^=text], #data tr:eq(" + rid + ") input[id^=text]").addClass("edited");
											statHit = 1;				
										}	
									}
								});
								...
								if (statHit == 0){
									...
									alert ("Baris yang berwarna biru memiliki jumlah pokok melebihi max sisa pokok !");
								}	
								//}
							//}
						//})
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( src_region_code == '' ) && ( key_find == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-manual-sisip/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_activity_code/" + src_activity_code + "/src_afd/" + src_afd,'_blank');
		}
    });
	...
	$("#pick_activity").click(function() {
		popup("pick/activity-mapp/module/rktManualSisip", "pick", 700, 400 );
    });
	...
	$(".val_dis").live("change", function(event) {
		...
		valmonth = Number($("#text20_"+row).val().replace(",", ""))+
					...
					Number($("#text31_"+row).val().replace(",", ""));
		
		inp = $(this).val();
		if (Number(max_sph) < 0){
			alert ("Maksimal Distribusi yang boleh diinput adalah 0!");
			$(this).val(0);
			$(this).focus();
		}else{
			if (valmonth > Number(max_sph)) {
				alert ("Maksimal Distribusi total yang boleh diinput adalah "+max_sph+" !");
				$(this).val(0);
				$(this).focus();
			}
		}
	...
	$("[id^=text16_]").live("change", function(event) {
		...
		$.ajax({
			type     : "post",
			url      : "rkt-manual-sisip/get-rotation",
	...
	/*$('#text48_').append(new Option('1 - JAN', '1'));
	$('#text48_').append(new Option('2 - FEB', '2'));
	$('#text48_').append(new Option('3 - MAR', '3'));
	$('#text48_').append(new Option('4 - APR', '4'));
	$('#text48_').append(new Option('5 - MAY', '5'));
	$('#text48_').append(new Option('6 - JUN', '6'));
	$('#text48_').append(new Option('7 - JUL', '7'));
	$('#text48_').append(new Option('8 - AUG', '8'));
	$('#text48_').append(new Option('9 - SEP', '9'));
	$('#text48_').append(new Option('10 - OCT', '10'));
	$('#text48_').append(new Option('11 - NOV', '11'));
	$('#text48_').append(new Option('12 - DEC', '12'));
	$('#text48_').append(new Option('-', '0'));*/
	
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "rkt-manual-sisip/upd-locked-seq-status",
    ...
    $.ajax({
        ...
        success : function(data) { 
			if (data.return == 'locked') {
				...
				statusval = 0; //untuk validasi jumlah pokok
				count = data.count;
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#data_freeze tr:eq(" + index + ") input[id^=maxNorma_]").val(Math.ceil(row.SPH_MAX)); //tambahan untuk validasi
						...
						$("#data tr:eq(" + index + ") input[id^=text20_]").val(accounting.formatNumber(row.PLAN_JAN, 0));
						$("#data tr:eq(" + index + ") input[id^=text20_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text21_]").val(accounting.formatNumber(row.PLAN_FEB, 0));
						$("#data tr:eq(" + index + ") input[id^=text21_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text22_]").val(accounting.formatNumber(row.PLAN_MAR, 0));
						$("#data tr:eq(" + index + ") input[id^=text22_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text23_]").val(accounting.formatNumber(row.PLAN_APR, 0));
						$("#data tr:eq(" + index + ") input[id^=text23_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text24_]").val(accounting.formatNumber(row.PLAN_MAY, 0));
						$("#data tr:eq(" + index + ") input[id^=text24_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text25_]").val(accounting.formatNumber(row.PLAN_JUN, 0));
						$("#data tr:eq(" + index + ") input[id^=text25_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text26_]").val(accounting.formatNumber(row.PLAN_JUL, 0));
						$("#data tr:eq(" + index + ") input[id^=text26_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text27_]").val(accounting.formatNumber(row.PLAN_AUG, 0));
						$("#data tr:eq(" + index + ") input[id^=text27_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text28_]").val(accounting.formatNumber(row.PLAN_SEP, 0));
						$("#data tr:eq(" + index + ") input[id^=text28_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text29_]").val(accounting.formatNumber(row.PLAN_OCT, 0));
						$("#data tr:eq(" + index + ") input[id^=text29_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text30_]").val(accounting.formatNumber(row.PLAN_NOV, 0));
						$("#data tr:eq(" + index + ") input[id^=text30_]").addClass('distribusi_kerja');
						$("#data tr:eq(" + index + ") input[id^=text31_]").val(accounting.formatNumber(row.PLAN_DEC, 0));
						...
						//validasi untuk total poko tidak lebih dari max sisa pokok
						totalPokok = Number(row.PLAN_JAN)+Number(row.PLAN_FEB)+Number(row.PLAN_MAR)+Number(row.PLAN_APR)+Number(row.PLAN_MAY)
									+Number(row.PLAN_JUN)+Number(row.PLAN_JUL)+Number(row.PLAN_AUG)+Number(row.PLAN_SEP)+Number(row.PLAN_OCT)
									+Number(row.PLAN_NOV)+Number(row.PLAN_DEC);

						if (totalPokok > Math.ceil(row.SPH_MAX)){					
							$("#data_freeze tr:eq(" + index + ") select[id^=text], #data_freeze tr:eq(" + index + ") input[id^=text]").addClass("edited");
							$("#data tr:eq(" + index + ") select[id^=text], #data tr:eq(" + index + ") input[id^=text]").addClass("edited");
							statusval = 1;				
						}					
