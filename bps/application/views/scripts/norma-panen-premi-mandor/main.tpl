...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Norma Panen Premi Mandor
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>NORMA PANEN PREMI MANDOR</legend>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr>
					<td width="15%"><b>YIELD BA :</b></td>
					<td width="85%">
						<input type="text" name="yield_ba" id="yield_ba" value="" style="width:70px;" readonly="readonly"/>
					</td>
				</tr>
			</table>
			<div id='scrollarea'>
			...
			<thead>
				<tr>
					...
					<th>MIN YIELD<BR>HA/BULAN</th>
					<th>MAX YIELD<BR>HA/BULAN</th>
					<th>OER MIN</th>
					<th>OER MAX</th>
					<th>RP/KG</th>
				</tr>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:175px' value='4'/></td>
					<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:175px' value='5'/></td>
					<td><input type="text" name="text06[]" id="text06_" readonly="readonly" style='width:175px' value='6'/></td>
					<td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:175px' value='7'/></td>
					<td><input type="text" name="text08[]" id="text08_" style='width:200px' value='8'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "norma-panen-premi-mandor/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "norma-panen-premi-mandor/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
							$("#btn_save_temp").hide();
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "norma-panen-premi-mandor/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "norma-panen-premi-mandor/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
								if(data.STATUS == 'LOCKED'){ 
									...
									$.ajax({
										type     : "post",
										url      : "norma-panen-premi-mandor/save",
										data     : $("#form_init").serialize(),
										cache    : false,
										dataType : 'json',
										success  : function(data) {
	...
	$("#btn_save_temp").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "norma-panen-premi-mandor/save-temp",
				data     : $("#form_init").serialize(),
				cache    : false,
				success  : function(data) {
					if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-norma-panen-premi-mandor/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "norma-panen-premi-mandor/upd-locked-seq-status",
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "norma-panen-premi-mandor/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
		if (data.return == 'locked') {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(accounting.formatNumber(row.MIN_YIELD, 2));
					$("#data tr:eq(" + index + ") input[id^=text04_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text05_]").val(accounting.formatNumber(row.MAX_YIELD, 2));
					$("#data tr:eq(" + index + ") input[id^=text05_]").addClass("number");
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.MIN_OER, 2));
					$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.MAX_OER, 2));
					...
					//asumsi over basis
					$("#yield_ba").val(accounting.formatNumber(row.YIELD, 2));
					$("#yield_ba").addClass("number");
                });
            }
		 }
        }
