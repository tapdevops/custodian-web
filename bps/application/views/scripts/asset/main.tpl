...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master Aset
Function 			:	
...
1		||	Doni R			||	12-06-2013		||	Menambahkan LOV Region
=========================================================================================================================
*/
		...
		<fieldset>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text05[]" id="text05_" style='width:500px;'/></td>
					...
					<td><input type="text" name="text09[]" id="text09_" style='width:100px;' title="Tekan F9 Untuk Memilih."/></td>
					<td><input type="text" name="text10[]" id="text10_" style='width:100px;'/></td>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "asset/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					...
					if(data==1){
							$.ajax({
								type    : "post",
								url     : "asset/check-locked-seq", //check apakah status lock sendiri apakah lock
								data    : $("#form_init").serialize(),
								cache   : false,
								dataType: "json",
								success : function(data) {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
			...
			if (rowid == '') {
				...
				$.ajax({
					type     : "post",
					url      : "asset/delete/rowid/"+encode64(rowid),
					cache    : false,
					//dataType : 'json',
					success  : function(data) {
						if (data == "done") {
	...
	$("#btn_export_csv").live("click", function() {	
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-asset/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/coa-capex/module/asset/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text08_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/uom/module/asset/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text09_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/asset-status/module/asset/row/" + row, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text11_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/norma-basic/module/asset/row/" + row + "/bacode/" + bacode, "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "asset/upd-locked-seq-status",
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.ASSET_CODE);
					...
					$("#data tr:eq(" + index + ") input[id^=text09_]").val(row.STATUS);
