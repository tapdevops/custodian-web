...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Report Koneksitas
Function 			:	
Disusun Oleh		: 	IT Enterprise Application - PT Triputra Agro Persada	
Developer			: 	Nicholas Budihardja
Dibuat Tanggal		: 	11/06/2015
Update Terakhir		:	11/06/2015
Revisi				:	
=========================================================================================================================
*/
				...
				<tr>
					...
					<td>
						<select name="jenis_report" id="jenis_report" style="width:205px;" />
							<option value="produksi" selected=selected>REPORT - PRODUKSI</option>
							<option value="hs">REPORT - HECTAR STATEMENT</option>
						</select>
					</td>
				</tr>				
				<tr>
					<td>&nbsp;</td>
					<td><i><font color="red">Pilih Jenis Report, Lalu Klik Download Excel</font></i></td>
				</tr>	
				<tr>
					<td></td>
					<td>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			switch(jenis_report){
				case "produksi":
				  window.open("<?=$_SERVER['PHP_SELF']?>/report-koneksitas/report-produksi/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find,'_blank');
				  break;
				case "hs":
				  window.open("<?=$_SERVER['PHP_SELF']?>/report-koneksitas/report-hs/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find,'_blank');
				  break; 
			}
	...
	$("#btn_generate").click( function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "report-koneksitas/generate-report",
	...
	/*$("#jenis_report").change( function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "report-koneksitas/get-last-generate",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType : 'json',
				success  : function(data) {
