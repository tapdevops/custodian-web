...
header("Content-Disposition: attachment;Filename=report_sum_estate_cost_".date("YmdHis").".xls");
...
<div style='font-weight:bolder; font-size:20px;'>SUMMARY ESTATE COST</div>
...
if($this->data['count'] > 0){
	foreach($this->data['rows'] as $idx => $row){	
		//total per tipe
		if( ($lastBa) && ($lastTipeTransaksi) &&($row['BA_CODE'].$row['GROUP02'] <> $lastBa.$lastTipeTransaksi) && ($lastActivity <> 'HA_TM')  && ($lastActivity <> 'PRODUKSI_TBS') ){
			//sub total
		...
		echo "<tr style='vertical-align:top;'>";
			...
			if($row['BA_CODE'].$row['GROUP02'] <> $lastBa.$lastTipeTransaksi){
