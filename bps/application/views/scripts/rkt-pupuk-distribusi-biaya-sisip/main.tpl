...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan RKT Pupuk Distribusi Biaya Sisip
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>PUPUK - DISTRIBUSI BIAYA SISIP</legend>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-sisip/get-status-periode",
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-sisip/save-all",
				data     : $("#form_init").serialize(),
				cache    : false,
				//dataType : 'json',
				success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-pupuk-distribusi-biaya-sisip/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_afd/" + src_afd + "/src_jenis_pupuk/" + src_jenis_pupuk,'_blank');
		}
    });	
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "rkt-pupuk-distribusi-biaya-sisip/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
