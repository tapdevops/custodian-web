...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master Aktivitas
Function 			:	
...
<div>
	<fieldset>
		<legend>MASTER AKTIVITAS</legend>
		<div id='scrollarea'>
		...
		<thead>
			<tr>
				...
				<th>DESKRIPSI</th>
				<th>AKTIVITAS INDUK</th>
				<th>DESKRIPSI AKTIVITAS INDUK</th>
				<th>SATUAN</th>
				<th>FLAG</th>
			</tr>
		...
		<tbody name='data' id='data'>
			<tr style="display:none">
				...
				<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:320px;'/></td>
				<td><input type="text" name="text04[]" id="text04_" readonly="readonly" style='width:150px;'/></td>
				<td><input type="text" name="text05[]" id="text05_" readonly="readonly" style='width:320px;'/></td>
...
$(document).ready(function() {
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "activity/upd-locked-seq-status",
...
function cek_squence(){
	$.ajax({
		type    : "post",
		url     : "activity/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
		...
		success : function(data) {
			if(data==1){
					$.ajax({
						type    : "post",
						url     : "activity/check-locked-seq", //check apakah status lock sendiri apakah lock
						data    : $("#form_init").serialize(),
						cache   : false,
						dataType: "json",
						success : function(data) {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.ACTIVITY_PARENT_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.ACTIVITY_PARENT_DESC);
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(row.UOM);
					$("#data tr:eq(" + index + ") input[id^=text07_]").val(row.FLAG);
