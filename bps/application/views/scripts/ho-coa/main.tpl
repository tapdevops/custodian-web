		...
		<fieldset>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" style="width:620px;" /></td>
...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")){
			$.ajax({
				type: 'POST',
				url: 'ho-coa/delete/rowid/' + encode64(rowid),
				cache: false,
				dataType: 'json',
				success: function(data) {
					if (data.return == "done") {
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.COA_NAME);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.COA_GROUP);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.STATUS);
