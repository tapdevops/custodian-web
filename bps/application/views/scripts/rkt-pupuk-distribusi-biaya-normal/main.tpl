...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan RKT Pupuk Distribusi Biaya Normal
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>PUPUK - DISTRIBUSI BIAYA NORMAL</legend>
			...
			<thead>
				<tr name='content' id='content'>
					...
					<th>DISTRIBUSI BIAYA PUPUK<BR>JAN</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>FEB</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>MAR</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>APR</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>MEI</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>JUN</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>JUL</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>AGS</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>SEP</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>OKT</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>NOV</th>
					<th>DISTRIBUSI BIAYA PUPUK<BR>DES</th>					
					<th>DISTRIBUSI BIAYA PUPUK<BR>YEAR</th>
					<th>TRANSPORT/KG</th>
					<th>TOOLS/KG</th>
					<th>LABOUR/KG</th>
			...
			<tbody name='data' id='data'>
				<tr name='content' id='content' style="display:none;" >
					...
					<td width='75px'><input type="text" name="text42[]" id="text42_" readonly="readonly" /></td>
					<td width='75px'><input type="text" name="text06[]" id="text06_" readonly="readonly" value='6'/></td>
					<td width='75px'><input type="text" name="text07[]" id="text07_" readonly="readonly" value='7'/></td>
					...
					<td width='150px'><input type="text" name="text39[]" id="text39_" readonly="readonly" value='39'/></td>
					<td width='150px'><input type="text" name="text40[]" id="text40_" readonly="readonly" value='40'/></td>
					<td width='150px'><input type="text" name="text41[]" id="text41_" readonly="readonly" value='41'/></td>
				</tr>			
			</tbody>
			<!--
			<tfoot name='tfoot' id='tfoot' style="display:none">
				<tr>
					...
					<td><input type="text" name="total27" id="total27" readonly="readonly" style='width:120px' value='27'/></td>
					<td><input type="text" name="total28" id="total28" readonly="readonly" style='width:120px' value='28'/></td>
					<td><input type="text" name="total29" id="total29" readonly="readonly" style='width:120px' value='29'/></td>
					...
					<td><input type="text" name="total39" id="total39" readonly="readonly" style='width:150px' value=''/></td>
					<td><input type="text" name="total40" id="total40" readonly="readonly" style='width:150px' value=''/></td>
					<td><input type="text" name="total41" id="total41" readonly="readonly" style='width:150px' value=''/></td>
				</tr>
			</tfoot>
			-->
			</table>
...
$(document).ready(function() {
	...
	$("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-normal/get-status-periode",
	...
	$("#btn_save").click( function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-pupuk-distribusi-biaya-normal/save-all",
				data     : $("#form_init").serialize(),
				cache    : false,
				//dataType : 'json',
				success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-pupuk-distribusi-biaya-normal/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_afd/" + src_afd + "/src_jenis_pupuk/" + src_jenis_pupuk,'_blank');
		}
    });	
...
function getData(){
	...
	var cost_transport = parseFloat(0);
	var cost_tools = parseFloat(0);
	var cost_labour = parseFloat(0);
	
    //
    $.ajax({
        type    : "post",
        url     : "rkt-pupuk-distribusi-biaya-normal/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) {
            ...
            if (count > 0) {
				$.each(data.rows, function(key, row) {
					...
					$("#data_freeze tr:eq(" + index + ") input[id^=text42_]").val(row.BLOCK_CODE + " - " + row.BLOCK_DESC);
					...
					$("#data tr:eq(" + index + ") input[id^=text26_]").val(accounting.formatNumber(row.SETAHUN, 2));
					$("#data tr:eq(" + index + ") input[id^=text26_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text27_]").val(accounting.formatNumber(row.COST_TRANSPORT_KG, 2));
					$("#data tr:eq(" + index + ") input[id^=text27_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text28_]").val(accounting.formatNumber(row.COST_TOOLS_KG, 2));
					$("#data tr:eq(" + index + ") input[id^=text28_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text29_]").val(accounting.formatNumber(row.COST_LABOUR_POKOK, 2));
					$("#data tr:eq(" + index + ") input[id^=text29_]").addClass("number");
					$("#data tr:eq(" + index + ") input[id^=text30_]").val(row.PUPUK_JAN);
					$("#data tr:eq(" + index + ") input[id^=text31_]").val(row.PUPUK_FEB);
					$("#data tr:eq(" + index + ") input[id^=text32_]").val(row.PUPUK_MAR);
					$("#data tr:eq(" + index + ") input[id^=text33_]").val(row.PUPUK_APR);
					$("#data tr:eq(" + index + ") input[id^=text34_]").val(row.PUPUK_MAY);
					$("#data tr:eq(" + index + ") input[id^=text35_]").val(row.PUPUK_JUN);
					$("#data tr:eq(" + index + ") input[id^=text36_]").val(row.PUPUK_JUL);
					$("#data tr:eq(" + index + ") input[id^=text37_]").val(row.PUPUK_AUG);
					$("#data tr:eq(" + index + ") input[id^=text38_]").val(row.PUPUK_SEP);
					$("#data tr:eq(" + index + ") input[id^=text39_]").val(row.PUPUK_OCT);
					$("#data tr:eq(" + index + ") input[id^=text40_]").val(row.PUPUK_NOV);
					$("#data tr:eq(" + index + ") input[id^=text41_]").val(row.PUPUK_DES);
					$("#data tr:eq(" + index + ")").removeAttr("style");					
                    $("#data tr:eq(1) input[id^=text02_]").focus();		

					//perhitungan total
					YEAR = (row.SETAHUN) ? YEAR + parseFloat(row.SETAHUN) : YEAR;
					...
					cost_transport = parseFloat(row.COST_TRANSPORT_KG);
					cost_tools = parseFloat(row.COST_TOOLS_KG);
					cost_labour = parseFloat(row.COST_LABOUR_POKOK);
                });
				/*
				...
				$("#total27").val(accounting.formatNumber(cost_transport, 2));
				$("#total27").addClass("number grandtotal_text");
				$("#total28").val(accounting.formatNumber(cost_tools, 2));
				$("#total28").addClass("number grandtotal_text");
				$("#total29").val(accounting.formatNumber(cost_labour, 2));
				$("#total29").addClass("number grandtotal_text");
				...
				$("#total39").addClass("grandtotal_text");
				$("#total40").addClass("grandtotal_text");
				$("#total41").addClass("grandtotal_text");
				$("#tfoot").show();
				*/
            }
			else{
