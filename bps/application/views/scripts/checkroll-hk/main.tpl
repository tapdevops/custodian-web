...
Versi				: 	1.0.0
Deskripsi			: 	View untuk Menampilkan Master Checkroll HK
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>MASTER CHECKROLL HK</legend>
			...
			<thead>
				<tr>
					...
					<th colspan='4'>JUMLAH HARI / TAHUN</th>
					<th colspan='4'>HARI NON EFEKTIF</th>
			...
			<tbody width='100%' name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text13[]" id="text13_" readonly="readonly" style='width:50px;'/></td>
					<td><input type="text" name="text04[]" id="text04_" style='width:110px;'/></td>
					<td><input type="text" name="text05[]" id="text05_" style='width:110px;'/></td>
					<td><input type="text" name="text06[]" id="text06_" style='width:110px;'/></td>
					<td><input type="text" name="text07[]" id="text07_" readonly="readonly" style='width:110px;'/></td>
					<td><input type="text" name="text08[]" id="text08_" style='width:110px;'/></td>
					<td><input type="text" name="text09[]" id="text09_" style='width:110px;'/></td>
					<td><input type="text" name="text10[]" id="text10_" style='width:110px;'/></td>
					<td><input type="text" name="text11[]" id="text11_" style='width:110px;'/></td>
					<td><input type="text" name="text12[]" id="text12_" readonly="readonly" style='width:110px;'/></td>
				</tr>
			</tbody>
			</table>
			</div>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			$.ajax({
				type    : "post",
				url     : "checkroll-hk/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					...
					if(data==1){
							$.ajax({
								type    : "post",
								url     : "checkroll-hk/check-locked-seq", //check apakah status lock sendiri apakah lock
								data    : $("#form_init").serialize(),
								cache   : false,
								dataType: "json",
								success : function(data) {
			...
			$.ajax({
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type    : "post",
							url     : "checkroll-hk/check-locked-seq",
							data    : $("#form_init").serialize(),
							cache   : false,
							dataType: "json",
							success : function(data) {
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "checkroll-hk/upd-locked-seq-status",
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text13_]").val(row.EMPLOYEE_STATUS);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(accounting.formatNumber(row.HARI_SETAHUN, 0));
					$("#data tr:eq(" + index + ") input[id^=text04_]").addClass("integer");
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(accounting.formatNumber(row.MINGGU_SETAHUN, 0));
					$("#data tr:eq(" + index + ") input[id^=text05_]").addClass("integer");
                    $("#data tr:eq(" + index + ") input[id^=text06_]").val(accounting.formatNumber(row.LIBUR_SETAHUN, 0));
					$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("integer");
                    $("#data tr:eq(" + index + ") input[id^=text07_]").val(accounting.formatNumber(row.HK, 0));
					$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text08_]").val(accounting.formatNumber(row.CUTI, 0));
					$("#data tr:eq(" + index + ") input[id^=text08_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text09_]").val(accounting.formatNumber(row.SAKIT, 0));
					$("#data tr:eq(" + index + ") input[id^=text09_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text10_]").val(accounting.formatNumber(row.IZIN, 0));
					$("#data tr:eq(" + index + ") input[id^=text10_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text11_]").val(accounting.formatNumber(row.HAID, 0));
					$("#data tr:eq(" + index + ") input[id^=text11_]").addClass("integer");
					$("#data tr:eq(" + index + ") input[id^=text12_]").val(accounting.formatNumber(row.HKE, 0));
