...
Versi				: 	2.0.0
Deskripsi			: 	View untuk Menampilkan RKT Kastrasi Sanitasi
Function 			:	
...
Revisi				:	
	NBU 05/05/2015	: 	- penutupan button lock & unlock di line 78 - 81
					    - penutupan pengecekan lock pada diri sendiri di line 350
...
<form name="form_init" id="form_init">
        ...
        <fieldset>
			...
			<table border="0" width="100%" cellpadding="0" cellspacing="0" id="main">
				...
				<tr>
					...
					<td>
						<?php 
							echo $this->setElement($this->input['src_activity_code']);
						?>
					</td>
				</tr>
				<tr>
	...
	<div>
		<fieldset>
			<legend>RKT KASTRASI SANITASI</legend>
			...
			<thead>
				<tr name='content' id='content'>
					...
					<th>ROTASI SMS2</th>
					<th>SUMBER BIAYA</th>
					<th>PLAN<BR>JAN <span class='activity_uom'></span></th>
					<th>PLAN<BR>FEB <span class='activity_uom'></span></th>
					<th>PLAN<BR>MAR <span class='activity_uom'></span></th>
					<th>PLAN<BR>APR <span class='activity_uom'></span></th>
					<th>PLAN<BR>MEI <span class='activity_uom'></span></th>
					<th>PLAN<BR>JUN <span class='activity_uom'></span></th>
					<th>PLAN<BR>JUL <span class='activity_uom'></span></th>
					<th>PLAN<BR>AGS <span class='activity_uom'></span></th>
					<th>PLAN<BR>SEP <span class='activity_uom'></span></th>
					<th>PLAN<BR>OKT <span class='activity_uom'></span></th>
					<th>PLAN<BR>NOV <span class='activity_uom'></span></th>
					<th>PLAN<BR>DES <span class='activity_uom'></span></th>					
					<th>PLAN<BR>YEAR <span class='activity_uom'></span></th>
					<th>TOTAL RP SMS1</th>
					<th>TOTAL RP SMS2</th>
					<th>COST<BR>JAN</th>
					<th>COST<BR>FEB</th>
			...
			<tbody width='100%' name='data' id='data'>
				<tr name='content' id='content' style="display:none;" >
					<td width='50px'>
						...
						<input type="hidden" name="hidden02[]" id="hidden02_" readonly="readonly"/>
					...
					<td width='50px'><input type="text" name="text081[]" id="text081_" readonly="readonly"/></td>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type     : "post",
				url      : "rkt-kastrasi-sanitasi/get-tipe-norma",
				...
				success  : function(data) {
					count = data.count;
					if (count > 0) {
						$('#text50_').find('option').remove().end();
						$.each(data.rows, function(key, row) {
							$('#text50_').append(new Option(row.NILAI,row.NILAI));
						});
					}
				}
			});	
			...
			$.ajax({
				type     : "post",
				url      : "rkt-kastrasi-sanitasi/get-activity-class",
				data     : $("#form_init").serialize(),
				cache    : false,
				dataType: "json",
				success  : function(data) {
					...
					if (count > 0) {
						...
						//set nilai norma umum/khusus
						/*$.each(data.rows, function(key, row) {
							$('#text50_').append(new Option("UMUM","UMUM"));
							$('#text50_').append(new Option("KHUSUS","KHUSUS"));
						});*/
						...
						$.ajax({
							...
							success : function(data) {
								if(data==1){
									...
									$.ajax({
										type     : "post",
										url      : "rkt-kastrasi-sanitasi/get-status-periode", //cek status periode
										data     : $("#form_init").serialize(),
										cache    : false,
										dataType: "json",
										success  : function(data) {
											...
											if (data == 'CLOSE') {
													...
													/*$.ajax({
														type    : "post",
														url     : "rkt-kastrasi-sanitasi/check-locked-seq", //check apakah status lock sendiri apakah lock
														data    : $("#form_init").serialize(),
														cache   : false,
														dataType: "json",
														success : function(data) {
						...
						clearDetail();
						alert("Kelas Aktivitas Tidak Tersedia.");
					}
				}
			});				
			...
			$.ajax({
					...
					success : function(data) {
						if(data==1){
							...
							/*$.ajax({
								type    : "post",
								url     : "rkt-kastrasi-sanitasi/check-locked-seq",
								data    : $("#form_init").serialize(),
								cache   : false,
								dataType: "json",
								success : function(data) {
									if(data.STATUS == 'LOCKED'){ 
										...
										$.ajax({
											type     : "post",
											url      : "rkt-kastrasi-sanitasi/save",
											data     : $("#form_init").serialize(),
											cache    : false,
											dataType : 'json',
											success  : function(data) {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( src_region_code == '' ) && ( key_find == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-kastrasi-sanitasi/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_matstage_code/" + src_matstage_code + "/src_activity_code/" + src_activity_code + "/src_afd/" + src_afd,'_blank');
		}
    });
	...
	$("#pick_activity").click(function() {
		popup("pick/activity-mapp/module/rktKastrasiSanitasi", "pick", 700, 400 );
    });
	$("#src_activity_desc").live("keydown", function(event) {
		//tekan F9
        if (event.keyCode == 120) {
			//lov
			popup("pick/afdeling-mapp/module/rktKastrasiSanitasi", "pick", 700, 400 );
        }else{
			event.preventDefault();
		}
    });
	...
	$("[id^=text16_]").live("change", function(event) {
		...
		$.ajax({
			type     : "post",
			url      : "rkt-manual-non-infra/get-rotation",
	...
	$("[id^=text50_]").live("change", function(event) {
		var row = $(this).attr("id").split("_")[1];
		$.ajax({
			type     : "post",
			url      : "rkt-kastrasi-sanitasi/get-rotation",
			data     : { BA_CODE: $("#text03_" + row).val(), 
						 MATURITY_STAGE_SMS1: $("#text09_" + row).val(), 
						 MATURITY_STAGE_SMS2: $("#text10_" + row).val(), 
						 ACTIVITY_CODE: $("#text14_" + row).val(),
						 ...
						 LAND_TYPE: $("#hidden00_" + row).val(), 
						 TOPOGRAPHY: $("#hidden01_" + row).val(), 
						 TIPE_NORMA: $("#text50_" + row).val() //<!-- TIPE NORMA -->
					   },
			cache    : false,
			dataType : 'json',
			success  : function(data) {
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "rkt-kastrasi-sanitasi/upd-locked-seq-status",
...
function getData(){
    ...
    $.ajax({
        type    : "post",
        url     : "rkt-kastrasi-sanitasi/list",
        data    : $("#form_init").serialize(),
        cache   : false,
        dataType: "json",
        success : function(data) { 
			if (data.return == 'locked') {
				...
				if (count > 0) {
					$.each(data.rows, function(key, row) {
						...
						$("#data_freeze tr:eq(" + index + ") input[id^=hidden02_]").val(row.LAND_SUITABILITY);
						...
						$("#data tr:eq(" + index + ") input[id^=text081_]").val(row.LAND_SUITABILITY);
						...
						$("#data tr:eq(" + index + ") input[id^=text15_]").val(row.ACTIVITY_DESC);
						...
						//$("#data tr:eq(" + index + ") input[id^=text33_]").val(row.TOTAL_RP_SMS1);
						$("#data tr:eq(" + index + ") input[id^=text33_]").val(accounting.formatNumber(row.TOTAL_RP_SMS1, 2));
						$("#data tr:eq(" + index + ") input[id^=text34_]").val(accounting.formatNumber(row.TOTAL_RP_SMS2, 2));
						...
						//$("#text48_" + index + " option[value='"+row.AWAL_ROTASI+"']").attr("selected", "selected"); // awal rotasi
