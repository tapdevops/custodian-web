		...
		<fieldset>
			...
			<tbody name='data' id='data'>
				<tr style="display:none">
					...
					<td><input type="text" name="text03[]" id="text03_" readonly="readonly" style='width:400px;'/></td>
        ...
        success : function(data) {
            ...
            if (count > 0) {
                $.each(data.rows, function(key, row) {
                    ...
                    $("#data tr:eq(" + index + ") input[id^=text02_]").val(row.COA_CODE);
                    $("#data tr:eq(" + index + ") input[id^=text03_]").val(row.DESCRIPTION);
                    $("#data tr:eq(" + index + ") input[id^=text04_]").val(row.COA_PARENT);
                    $("#data tr:eq(" + index + ") input[id^=text05_]").val(row.COA_PARENT_DESC);
					$("#data tr:eq(" + index + ") input[id^=text06_]").val(row.FLAG);
