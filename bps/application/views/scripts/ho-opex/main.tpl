...
$(document).ready(function() {
	...
	$("input[id^=btn01_]").live("click", function(event) {
		...
		if(confirm("Apakah Anda Yakin Untuk Menghapus Data ke - " + row + " ?")) {
			if (rowid == '') {
				...
				$.ajax({
					type: 'POST',
					url: 'ho-opex/delete/rowid/' + encode64(rowid),
					cache: false,
					dataType: 'json',
    ...
    $('#pick_div').click(function() {
    	...
    	popup("pick/ho-division/module/hoOpex/ac/<?php echo $newdiv; ?>/us/<?php echo $this->username; ?>", "pick", 700, 400);
    });
	...
	$("input[id^=text06_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/ho-rencana-kerja/module/hoOpex/cc/" + cc + "/row/" + row, "pick");
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text08_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/ho-core/module/hoOpex/row/" + row, "pick");
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text10_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/ho-company/module/hoOpex/core/" + core + "/row/" + row, "pick");
        }else{
			event.preventDefault();
		}
    });
	$("input[id^=text13_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/ho-coa/module/hoOpex/row/" + row + "/core/" + core, "pick");
        }else{
			event.preventDefault();
		}
    });
				...
				if (count < 1) {
					...
					if (count > 0) {
						$.each(data.rows, function(key, row) {
							...
							$("#data tr:eq(" + index + ") input[id^=text06_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text07_]").val(row.OPEX_DESCRIPTION);
							$("#data tr:eq(" + index + ") input[id^=text07_]").addClass("required");
							...
							$("#data tr:eq(" + index + ") input[id^=text15_]").val(accounting.formatNumber(row.OPEX_JAN, 2));
							$("#data tr:eq(" + index + ") input[id^=text15_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text16_]").val(accounting.formatNumber(row.OPEX_FEB, 2));
							$("#data tr:eq(" + index + ") input[id^=text16_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text17_]").val(accounting.formatNumber(row.OPEX_MAR, 2));
							$("#data tr:eq(" + index + ") input[id^=text17_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text18_]").val(accounting.formatNumber(row.OPEX_APR, 2));
							$("#data tr:eq(" + index + ") input[id^=text18_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text19_]").val(accounting.formatNumber(row.OPEX_MAY, 2));
							$("#data tr:eq(" + index + ") input[id^=text19_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text20_]").val(accounting.formatNumber(row.OPEX_JUN, 2));
							$("#data tr:eq(" + index + ") input[id^=text20_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text21_]").val(accounting.formatNumber(row.OPEX_JUL, 2));
							$("#data tr:eq(" + index + ") input[id^=text21_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text22_]").val(accounting.formatNumber(row.OPEX_AUG, 2));
							$("#data tr:eq(" + index + ") input[id^=text22_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text23_]").val(accounting.formatNumber(row.OPEX_SEP, 2));
							$("#data tr:eq(" + index + ") input[id^=text23_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text24_]").val(accounting.formatNumber(row.OPEX_OCT, 2));
							$("#data tr:eq(" + index + ") input[id^=text24_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text25_]").val(accounting.formatNumber(row.OPEX_NOV, 2));
							$("#data tr:eq(" + index + ") input[id^=text25_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text26_]").val(accounting.formatNumber(row.OPEX_DEC, 2));
							$("#data tr:eq(" + index + ") input[id^=text26_]").addClass("required");
							$("#data tr:eq(" + index + ") input[id^=text27_]").val(accounting.formatNumber(row.OPEX_TOTAL, 2));
							$("#data tr:eq(" + index + ") input[id^=tChange_]").val("");
							$("#data tr:eq(" + index + ")").removeAttr("style");
