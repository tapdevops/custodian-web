...
Versi				: 	2.0.0
Deskripsi			: 	View untuk Menampilkan RKT SHE
Function 			:	
...
<form name="form_init" id="form_init">
	...
	<div>
		<fieldset>
			<legend>RKT SUSTAINABILITY</legend>
...
$(document).ready(function() {
    ...
    $("#btn_find").click(function() {
		...
		if ( ba_code == '' ) {
			...
			$.ajax({
				type    : "post",
				url     : "rkt-she/chk-enh-locked-sequence", //check apakah urutan input didepannya ada yang belum lock
				...
				success : function(data) {
					if(data==1){
						...
						$.ajax({
							type     : "post",
							url      : "rkt-she/get-status-periode", //cek status periode
							data     : $("#form_init").serialize(),
							cache    : false,
							dataType: "json",
							success  : function(data) {
							$("#btn_save_temp").hide();
								if (data == 'CLOSE') {
										...
										$.ajax({
											type    : "post",
											url     : "rkt-she/check-locked-seq", //check apakah status lock sendiri apakah lock
											data    : $("#form_init").serialize(),
											cache   : false,
											dataType: "json",
											success : function(data) {
	...
	$("#btn_export_csv").live("click", function() {
		...
		if ( ( reference_role == 'BA_CODE' ) && ( region == '' ) && ( ba_code == '' ) ) {
			...
			window.open("<?=$_SERVER['PHP_SELF']?>/download/data-rkt-she/budgetperiod/" + budgetperiod + "/src_region_code/" + src_region_code + "/key_find/" + key_find + "/src_coa_code/" + src_coa_code + "/src_group_code/" + src_group_code + "/search/" + encode64(search),'_blank');
		}
    });
	...
	$("#pick_coa").click(function() {
		popup("pick/coa-relation/type/she", "pick", 700, 400 );
    });
	...
	$("input[id^=text09_]").live("keydown", function(event) {
        ...
        if (event.keyCode == 120) {
			//lov
			popup("pick/sub-group-relation/module/rktShe/row/" + row + "/type/she/coa/" + coa + "/group/" + group, "pick");
        }else{
			event.preventDefault();
		}
    });
	...
	$("#btn_lock").live("click", function(event) {
		if(confirm("Anda Yakin Untuk Melakukan Finalisasi Data?")){
			$.ajax({
				type     : "post",
				url      : "rkt-she/upd-locked-seq-status",
...
function setDefaultField(index){
	...
	var trxCode = genTransactionCode(budgetperiod, key_find, 'SHE');
