...
class Application_Model_HoCoa
{
    ...
    public function getList($params = array()) {
        ...
        if ($params['key_find'] != '') {
            $query .= "
                AND (
                    UPPER(COA_CODE) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(COA_NAME) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(COA_GROUP) LIKE UPPER('%".$params['key_find']."%')
    ...
    { 
        ...
        $coa_code = (empty($row['COA_CODE'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['COA_CODE']);
        $coa_name = (empty($row['COA_NAME'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['COA_NAME']);
        $coa_group = (empty($row['COA_GROUP'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['COA_GROUP']);
