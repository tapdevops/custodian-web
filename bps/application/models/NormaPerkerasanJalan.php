...
class Application_Model_NormaPerkerasanJalan
{
    ...
    public function getData($params = array())
    {
		$query = "
         ...
         TNPJ.VRA_CODE_DT,
         NVL (
            (/* remark doni, salah ambil, harusnya dari tn_perkerasan_jalan*/ 
            (SELECT VALUE
            ...
            /*(SELECT max(rp_km_dt) kembali ke semula by YUS
                 FROM tn_perkerasan_jalan VRASUM
                WHERE     TNPJ.PERIOD_BUDGET = VRASUM.PERIOD_BUDGET
                      AND TNPJ.BA_CODE = VRASUM.BA_CODE
                      )*/
             UNION
	...
	//function untuk mengambil value VRA
	public function getVraPinjamValue($params = array())
    {
		$query = "
			SELECT RP_QTY AS VALUE
			FROM TN_VRA_PINJAM
			WHERE 1 = 1 AND VRA_CODE = '".$params['vra_code']."'
