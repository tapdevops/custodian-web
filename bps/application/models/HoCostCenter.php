...
=========================================================================================================================
*/
class Application_Model_HoCostCenter
{
    private $_db = null;
    ...
    public function getData($params = array()) {
        $query = "
            SELECT 
            ROWIDTOCHAR (THCC.ROWID) row_id,
            ROWNUM,
            THCC.HCC_ID,
            THCC.HCC_CC,
            THCC.HCC_DIVISI,
            THD.DIV_NAME,
            THCC.HCC_DIVISION_HEAD,
            THCC.HCC_COST_CENTER,
            THCC.HCC_COST_CENTER_HEAD
            FROM TM_HO_COST_CENTER THCC
            LEFT JOIN TM_HO_DIVISION THD
            ON THD.DIV_CODE = THCC.HCC_DIVISI
            WHERE THCC.DELETE_USER IS NULL
        ...
        if ($params['key_find'] != '') {
            $query .= "
                AND (
                    UPPER(HCC_CC) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(HCC_DIVISI) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(HCC_DIVISION_HEAD) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(HCC_COST_CENTER) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(HCC_COST_CENTER_HEAD) LIKE UPPER('%".$params['key_find']."')
                )
            ";
        }
    ...
    { 
        ...
        $hcc_cc = (empty($row['HCC_CC'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['HCC_CC']);
        $hcc_divisi = (empty($row['HCC_DIVISI'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['HCC_DIVISI']);
        $hcc_cost_center = (empty($row['HCC_COST_CENTER'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['HCC_COST_CENTER']);
        $hcc_cost_center_head = (empty($row['HCC_COST_CENTER_HEAD'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['HCC_COST_CENTER_HEAD']);
        $hcc_division_head = (empty($row['HCC_DIVISION_HEAD'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['HCC_DIVISION_HEAD']);

        $sql = "
            UPDATE TM_HO_COST_CENTER
            SET 
                HCC_CC = '".$hcc_cc."',
                HCC_DIVISI = '".$hcc_divisi."',
                HCC_COST_CENTER = '".$hcc_cost_center."',
                HCC_COST_CENTER_HEAD = '".$hcc_cost_center_head."',
                HCC_DIVISION_HEAD = '".$hcc_division_head."',
