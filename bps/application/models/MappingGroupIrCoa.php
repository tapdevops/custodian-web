...
Versi				: 	1.0.0
Deskripsi			: 	Model Class untuk Mapping Group Internal Relation dan COA
Function 			:	- getData			: ambil data dari DB
						- getList			: menampilkan list Mapping Group Internal Relation dan COA
						- save				: simpan data
						- delete			: hapus data
...
=========================================================================================================================
*/
class Application_Model_MappingGroupIrCoa
{
    private $_db = null;
	...
	//menampilkan list Mapping Group Internal Relation dan COA
    ...
    { 
		...
		if ($row['ROW_ID']){
			try {
				...
				$this->_db->commit();
				//log DB
				$this->_global->insertLog('UPDATE SUCCESS', 'MAPPING GRUP INTERNAL RELATION - COA', '', '');
			} catch (Exception $e) {
				//log DB
				$this->_global->insertLog('UPDATE FAILED', 'MAPPING GRUP INTERNAL RELATION - COA', '', $e->getCode());
			...
			try {
				...
				$this->_db->commit();
				//log DB
				$this->_global->insertLog('INSERT SUCCESS', 'MAPPING GRUP INTERNAL RELATION - COA', '', '');
			} catch (Exception $e) {
				//log DB
				$this->_global->insertLog('INSERT FAILED', 'MAPPING GRUP INTERNAL RELATION - COA', '', $e->getCode());
