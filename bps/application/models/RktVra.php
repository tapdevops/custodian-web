...
class Application_Model_RktVra
{
    ...
    public function getData($params = array())
    {
        ...
        $query .= "
            ORDER BY rkt.BA_CODE, RKT.VRA_CODE, RKT.INTERNAL_ORDER, RKT.VRA_SUB_CAT_DESCRIPTION
        ";
    ...
    public function getDataDownload($params = array())
    {
        $query = "
            SELECT ROWIDTOCHAR(report.ROWID) row_id, rownum, 
                   ...
                   report.RVRA20_VALUE3,
                   vra_sum.VALUE RP_QTY_VRA_TYPE
            FROM BPS_PROD.TR_RKT_VRA report
            LEFT JOIN BPS_PROD.TM_VRA master_vra ON report.VRA_CODE = master_vra.VRA_CODE
            LEFT JOIN BPS_PROD.TR_RKT_VRA_SUM vra_sum ON report.BA_CODE = vra_sum.BA_CODE
              AND report.PERIOD_BUDGET = vra_sum.PERIOD_BUDGET AND report.VRA_CODE = vra_sum.VRA_CODE
            LEFT JOIN BPS_PROD.TM_ORGANIZATION ORG ON report.BA_CODE = ORG.BA_CODE
            WHERE report.DELETE_USER IS NULL
        ";
        ...
        $query .= "
            ORDER BY report.BA_CODE, master_vra.VRA_CODE, report.INTERNAL_ORDER, master_vra.VRA_SUB_CAT_DESCRIPTION
        ";
    ...
    public function delete($row = array())
    {
        ...
        $sql = "
            ...
            WHERE PERIOD_BUDGET = TO_DATE('01-01-{$row['PERIOD_BUDGET']}','DD-MM-RRRR') 
                ...
                AND ROWIDTOCHAR (ROWID) = '{$row['ROW_ID']}';
        ";
