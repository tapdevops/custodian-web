...
=========================================================================================================================
*/
class Application_Model_NormaBasic
{
    private $_db = null;
    ...
    public function getData($params = array())
    {
		...
		$query .= "
			ORDER BY A.BA_CODE, A.BASIC_NORMA_CODE
		";
	...
	public function getDeletedRec($params = array())
    {
        ...
        $sql = "
			...
			WHERE PERIOD_BUDGET = TO_DATE('01-01-{$params['PERIOD_BUDGET']}','DD-MM-RRRR') 
				...
				AND BASIC_NORMA_CODE = '{$params['BASIC_NORMA_CODE']}'
		";        
    ...
    { 
		...
		$sql = "
			UPDATE TN_BASIC
			SET  
				PERCENT_INCREASE = REPLACE('{$row['PERCENT_INCREASE']}',',',''),
		...
		if ($row['BASIC_NORMA_CODE'] == 'NC001'){
			...
			if (!empty($records1)) {
				try {				
					...
					//log DB
					$this->_global->insertLog('UPDATE SUCCESS', 'RKT CHECKROLL', '', '');
				} catch (Exception $e) {
					//log DB
