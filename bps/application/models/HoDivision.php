...
=========================================================================================================================
*/
class Application_Model_HoDivision
{
    private $_db = null;
    ...
    public function getData($params = array()) {
        $query = "
            ...
            ROWNUM,
            TO_CHAR(PERIOD_BUDGET, 'RRRR') PERIOD_BUDGET,
            DIV_CODE,
            DIV_NAME
            FROM TM_HO_DIVISION
            WHERE DELETE_USER IS NULL
        ...
        if ($params['key_find'] != '') {
            $query .= "
                AND (
                    TO_CHAR(PERIOD_BUDGET, 'RRRR') LIKE ('%".$params['key_find']."%')
                    OR UPPER(DIV_CODE) LIKE UPPER('%".$params['key_find']."%')
                    OR UPPER(DIV_NAME) LIKE UPPER('%".$params['key_find']."%')
                )
            ";
        }
    ...
    { 
        ...
        $div_name = (empty($row['DIV_NAME'])) ? '' : str_replace(array('.', "'", '&'), array('', "''", '\'||\'&\'||\''), $row['DIV_NAME']);

        $sql = "
            UPDATE TM_HO_DIVISION
            SET 
                PERIOD_BUDGET = TO_DATE('{$row['PERIOD_BUDGET']}', 'RRRR'),
                DIV_CODE = '".$div_code."',
                DIV_NAME = '".$div_name."',
