...
class Application_Model_ReportKebAktivitas
{
  ...
  -- INI UNTUK PERHITUNGAN SUPERVISI  5101030701 BONGKAR MUAT LABOUR
    ...
    // yaddi.surahman@tap-agri.co.id
    // 2017-08-30
    // perhitungan report's components fertilizer (pupuk tunggal dan  majemuk)
    ...
    $components['transport_pupuk_tunggal'] = "
      ...
      SELECT * FROM (
        SELECT S.*, SUM(S.JUMLAH) OVER (PARTITION BY PERIOD_BUDGET, BA_CODE, BLOCK_CODE, AFD_CODE, VRA_SUB_CAT_DESCRIPTION) QTY_TOTAL
          , SUM(S.JUMLAH/KG_AFDELING*PRICE_HM_KM) OVER (PARTITION BY PERIOD_BUDGET, BA_CODE, BLOCK_CODE, AFD_CODE, VRA_SUB_CAT_DESCRIPTION) COST_TOTAL 
        FROM (
          ...
          , '".$this->_userName."', CURRENT_TIMESTAMP, NULL
          , CAST(TM.BULAN_PEMUPUKAN AS INT) BULAN_PEMUPUKAN, VRA.PRICE_HM_KM
      ...
      PIVOT (
        SUM(JUMLAH) DIS, SUM(JUMLAH/KG_AFDELING*PRICE_HM_KM) COST
        FOR BULAN_PEMUPUKAN IN (
        '1' AS JAN, '2' AS FEB, '3' AS MAR, '4' AS APR, '5' AS MAY, '6' AS JUN,
        '7' AS JUL, '8' AS AUG, '9' AS SEP, '10' AS OCT, '11' AS NOV, '12' AS DEC
        )
    ...
    $components['perkerasan_jalan_material'] = "
      INSERT INTO TMP_RPT_KEB_EST_COST_BLOCK (
        ...
        SELECT RKT.PERIOD_BUDGET,
            ...
            RKT.DIS_JAN+RKT.DIS_FEB+RKT.DIS_MAR+RKT.DIS_APR+
            RKT.DIS_MAY+RKT.DIS_JUN+RKT.DIS_JUL+RKT.DIS_AUG+
            RKT.DIS_SEP+RKT.DIS_OCT+RKT.DIS_NOV+RKT.DIS_DEC COST_SETAHUN,
            '".$this->_userName."', CURRENT_TIMESTAMP
   ...
   --PERHITUNGAN PUPUK MAJEMUK & TUNGGAL (MATERIAL)
       ...
       FROM (       
          SELECT RKT.PERIOD_BUDGET,
             ...
             RKT.MATERIAL_NAME AS MATERIAL_NAME,
             '' KETERANGAN,
             'KG' UOM,
             0  AS QTY_JAN,
             0  AS QTY_FEB,
          ...
          FROM V_KG_PUPUK_PER_MATERIAL_BLK RKT
   ...
   UNION ALL
            ...
            SYSDATE AS INSERT_TIME
               FROM (SELECT RKT.PERIOD_BUDGET,
                            ...
                            'HK' UOM,
                            (RKT.QTY_TUNGGAL_JAN * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_JAN,
                            (RKT.QTY_TUNGGAL_FEB * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_FEB,
                            (RKT.QTY_TUNGGAL_MAR * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_MAR,
                            (RKT.QTY_TUNGGAL_APR * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_APR,
                            (RKT.QTY_TUNGGAL_MAY * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_MAY,
                            (RKT.QTY_TUNGGAL_JUN * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_JUN,
                            (RKT.QTY_TUNGGAL_JUL * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_JUL,
                            (RKT.QTY_TUNGGAL_AUG * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_AUG,
                            (RKT.QTY_TUNGGAL_SEP * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_SEP,
                            (RKT.QTY_TUNGGAL_OCT * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_OCT,
                            (RKT.QTY_TUNGGAL_NOV * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_NOV,
                            (RKT.QTY_TUNGGAL_DEC * RP_QTY_INTERNAL)
                            / NVL (CHECKROLL.RP_HK, 1)
                               AS QTY_DEC,
                       ...
                       FROM V_PKK_PUPUK_PER_MATERIAL_BLK RKT
                            LEFT JOIN TM_ORGANIZATION ORG
                               ON ORG.BA_CODE = RKT.BA_CODE
                            LEFT JOIN TN_INFRASTRUKTUR TNI
    ...
    -- PUPUK MAJEMUK LABOUR
      ...
      FROM (SELECT RKT.PERIOD_BUDGET,
             ...
             RKT.MATERIAL_NAME AS KETERANGAN,
             'HK' UOM,
             (RKT.QTY_MAJEMUK_JAN * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_JAN,
             (RKT.QTY_MAJEMUK_FEB * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_FEB,
             (RKT.QTY_MAJEMUK_MAR * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_MAR,
             (RKT.QTY_MAJEMUK_APR * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_APR,
             (RKT.QTY_MAJEMUK_MAY * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_MAY,
             (RKT.QTY_MAJEMUK_JUN * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_JUN,
             (RKT.QTY_MAJEMUK_JUL * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_JUL,
             (RKT.QTY_MAJEMUK_AUG * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_AUG,
             (RKT.QTY_MAJEMUK_SEP * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_SEP,
             (RKT.QTY_MAJEMUK_OCT * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_OCT,
             (RKT.QTY_MAJEMUK_NOV * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_NOV,
             (RKT.QTY_MAJEMUK_DEC * RP_QTY_INTERNAL)
             / NVL (CHECKROLL.RP_HK, 1)
              AS QTY_DEC,
