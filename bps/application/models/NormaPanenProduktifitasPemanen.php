...
Versi				: 	1.0.0
Deskripsi			: 	Model Class untuk Norma Panen Produktifitas Pemanen
Function 			:	- getList					: menampilkan list norma Panen Produktifitas Pemanen
						- save						: simpan data
...
=========================================================================================================================
*/
class Application_Model_NormaPanenProduktifitasPemanen
{
    private $_db = null;
    ...
    public function getData($params = array())
    {
		$query = "
			SELECT ROWIDTOCHAR (norma.ROWID) row_id,
				   ...
				   norma.TOPOGRAPHY, 
				   norma.VALUE
			FROM TN_PANEN_PROD_PEMANEN norma
    ...
    { 
        $result = true;
			...
			$sql = "UPDATE TN_PANEN_PROD_PEMANEN
					SET UMUR = ".$row['UMUR'].",
						VALUE = ".$row['VALUE'].",
						FLAG_TEMP = NULL,
						UPDATE_USER = '{$this->_userName}',
    ...
    { 		
		...
		if (!empty($records1)) {
			try {		
				...
				//log DB
				$this->_global->insertLog('UPDATE SUCCESS', 'RKT PANEN', '', 'UPDATED FROM NORMA PANEN - PRODUKTIFITAS PEMANEN');
			} catch (Exception $e) {
				//log DB
