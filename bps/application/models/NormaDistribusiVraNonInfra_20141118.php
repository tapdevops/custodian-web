...
class Application_Model_NormaDistribusiVraNonInfra
{
    ...
    {
		$query = "
			SELECT TRX_CODE,LOCATION_CODE, FLAG_TEMP, SUM(HM_KM) HM_KM, SUM(PRICE_HM_KM) PRICE_HM_KM 
			FROM TR_RKT_VRA_DISTRIBUSI VRA 
			LEFT JOIN TM_ACTIVITY TA 
				ON TA.ACTIVITY_CODE = VRA.ACTIVITY_CODE 
		...
		$query .= "GROUP BY TRX_CODE, LOCATION_CODE, FLAG_TEMP
			ORDER BY 1"; 
