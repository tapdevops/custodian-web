...
Versi				: 	1.0.0
Deskripsi			: 	Controller Class untuk Norma Panen Premi Topography
Function 			:	- listAction		: menampilkan list norma Panen Premi Topography
...
=========================================================================================================================
*/
class NormaPanenPremiTopographyController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPanenPremiTopography();
    ...
    {
        $this->_redirect('/norma-panen-premi-topography/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Panen Premi Topography';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['PERCENTAGE']		= $params['text05'][$key]; // PERCENTAGE
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {		
			...
			$urutan++;
			$filename = $uniq_code_file.'_00_TNPANENTOPO_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_saveTemp';
		...
		//generate filename untuk .sh dan .sql
		$urutan++;
		$filename = $uniq_code_file.'_00_TNPANENTOPO_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_save';
		$this->_global->createBashFile($filename); //create bash file
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {	
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENTOPO_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanen';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENTOPO_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenTotalCost';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENTOPO_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenSaveVRA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
	...
	public function saveTempAction()
    {
		...
		if (!empty($rows)) {		
			// ************************************************ SAVE NORMA PANEN PREMI TOPOGRAPHY TEMP ************************************************
			//generate filename untuk .sh dan .sql
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PANEN_PREMI_TOPOGRAPHY";
