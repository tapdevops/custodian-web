...
Versi				: 	1.0.0
Deskripsi			: 	Controller Class untuk Norma Panen Produktifitas Pemanen
Function 			:	- listAction		: menampilkan list norma Panen Produktifitas Pemanen
...
=========================================================================================================================
*/
class NormaPanenProduktifitasPemanenController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPanenProduktifitasPemanen();
    ...
    {
        $this->_redirect('/norma-panen-produktifitas-pemanen/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Panen Produktifitas Pemanen';
	...
	//menampilkan list norma Panen Produktifitas Pemanen
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['TOPOGRAPHY']		= $params['text04'][$key]; // TOPOGRAPHY
				$rows[$key]['UMUR']				= $params['text05'][$key]; // UMUR
				$rows[$key]['VALUE']			= $params['text06'][$key]; // VALUE
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {		
			...
			// ************************************************ SAVE NORMA PANEN TOPO TEMP ************************************************
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $uniq_code_file.'_00_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'saveTemp';
		...
		// ************************************************ SAVE ALL NORMA PANEN TOPO ************************************************
		//generate filename untuk .sh dan .sql
		$urutan++;
		$filename = $uniq_code_file.'_00_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'save';
		$this->_global->createBashFile($filename); //create bash file
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {	
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanen';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenTotalCost';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenSaveVRA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//tambahan aries untuk norma alat kerja panen
			// ************************************************ UPDATE NORMA NORMA ALAT KERJA PANEN ************************************************
			$model = new Application_Model_NormaAlatKerjaPanen();
			$rec = $this->_db->fetchAll("{$model->getData($row)}");
			
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'NALATKERJAPANEN';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRPM_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'TNALATKERJAPANEN_SUM';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			if (!empty($rec)) {
				foreach ($rec as $idx1 => $record1) {
					if(($lastBa) && ($lastBa <> $record1['BA_CODE'])){
						...
						$model->updateSummaryNormaAlatKerjaPanen($uPar);
					}
	...
	public function saveTempAction()
    {
		...
		if (!empty($rows)) {		
			// ************************************************ SAVE NORMA PANEN Produktifitas Pemanen TEMP ************************************************
			//generate filename untuk .sh dan .sql
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PANEN_PROD_PEMANEN";
