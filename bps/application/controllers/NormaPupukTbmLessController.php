...
Versi				: 	2.0.0
Deskripsi			: 	Controller Class untuk Norma Pupuk TBM
Function 			:	- getStatusPeriodeAction		: BDJ 22/07/2013	: cek status periode budget yang dipilih
						- listAction					: SID 24/06/2013	: menampilkan list norma Pupuk TBM
...
=========================================================================================================================
*/
class NormaPupukTbmLessController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPupukTbmLess();
    ...
    {
        $this->_redirect('/norma-pupuk-tbm-less/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Pupuk TBM';
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {
			//generate filename untuk .sh dan .sql
			$filename = $uniq_code_file.'_00_NPUPUKTBM_01_SAVETEMP';
			$this->_global->createBashFile($filename); //create bash file
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
		...
		// ************************************************ SAVE ALL NORMA PUPUK TBM ************************************************
		//generate filename untuk .sh dan .sql
		$filename = $uniq_code_file.'_00_NPUPUKTBM_02_SAVE';
		$this->_global->createBashFile($filename); //create bash file		
		$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
		...
		if (!empty($updated_row)) {
			...
			foreach ($updated_row as $idx1 => $record1) {
				...
				$record1['land_type'] 			= $record1['LAND_TYPE']; // LAND_TYPE	
				$record1['maturity_stage'] 		= $record1['MATURITY_STAGE']; // MATURITY_STAGE		
	...
	public function deleteAction()
    {
		...
		// ************************************************ UPDATE NORMA PUPUK ************************************************
		//generate filename untuk .sh dan .sql
		$filename = $uniq_code_file.'_00_NPUPUKTBM_01_DELETE';
		$this->_global->createBashFile($filename); //create bash file
		$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
		...
		// ************************************************ UPDATE INHERIT MODULE ************************************************
		$params['filename'] = $uniq_code_file.'_00_NPUPUKTBM_02_DELETE_INHERIT_DATA';
		...
		$params['maturity_stage'] 		= $params['MATURITY_STAGE']; // MATURITY_STAGE	
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {		
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_01_RKTPUPUKHA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_02_RKTPUPUKKGNORMAL';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_03_RKTPUPUKKGSISIP';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_04_RKTPUPUKBIAYANORMAL';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_05_RKTPUPUKBIAYASISIP';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$filename = $row['uniq_code_file'].'_NPUPUKTBM_06_RKTPUPUKBIAYA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PUPUK_TMBM2_LESS";
