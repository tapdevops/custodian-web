...
Versi				: 	2.0.0
Deskripsi			: 	Controller Class untuk RKT Manual - Non Infra
Function 			:	- getStatusPeriodeAction		: BDJ 23/07/2013	: cek status periode budget yang dipilih
						- listAction					: SID 23/07/2013	: menampilkan list RKT Manual - Non Infra
						- getActivityClassAction		: YIR 23/07/2013	: menampilkan list activity class
						- getTipeNormaAction			: SID 18/07/2013	: menampilkan list tipe norma
						- getRotationAction				: SID 23/07/2013	: get Rp/Rotasi SMS1 & SMS2
						- mappingAction					: SID 22/07/2013	: mapping textfield name terhadap field name di DB
						- saveAction					: SID 22/07/2013	: save data
						- saveTempAction				: SID 22/07/2013	: simpan data sementara sesuai input user
Disusun Oleh		: 	IT Enterprise Solution - PT Triputra Agro Persada
Developer			: 	Sabrina Ingrid Davita
Dibuat Tanggal		: 	22/07/2013
Update Terakhir		:	19/06/2014
Revisi				:	
	SID 19/06/2014	: 	- perubahan mekanisme penyimpananan data -> ditampung di .sql terlebih dahulu
	...
	SID 18/07/2013	: 	- penambahan getTipeNormaAction
	YUL 11/08/2014	: 	- tambah validasi sequence pada function chkEnhLockedSequenceAction dan checkLockedSeqAction
=========================================================================================================================
*/
class RktManualSisipController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_RktManualSisip();
    ...
    {
        $this->_redirect('/rkt-manual-non-infra/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Budgeting Tahap 2 &raquo; RKT RAWAT SISIP';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if ($key > 0){
				...
				$rows[$key]['AWAL_ROTASI']  		= $params['text48'][$key]; // AWAL_ROTASI
	...
	public function saveAction()
    {
		...
		$filename = $this->_global->genFileName()."SAVEROTATION";
		...
		//generate filename untuk .sh dan .sql
		$filename = $this->_global->genFileName()."COSTELEMENT";
		$this->_global->createBashFile($filename); //create bash file		
		$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
