...
Versi				: 	1.0.0
Deskripsi			: 	Controller Class untuk Norma Panen Loading
Function 			:	- listAction		: menampilkan list norma Panen Loading
...
=========================================================================================================================
*/
class NormaPanenLoadingController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPanenLoading();
    ...
    {
        $this->_redirect('/norma-panen-loading/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Panen Loading';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['JARAK_PKS_MIN']	= $params['text04'][$key]; // JARAK_PKS_MIN
				$rows[$key]['JARAK_PKS_MAX']	= $params['text05'][$key]; // JARAK_PKS_MAX
				$rows[$key]['TARGET_ANGKUT_TM_SUPIR']	= $params['text06'][$key]; // TARGET_ANGKUT_TM_SUPIR
				$rows[$key]['BASIS_TM_SUPIR']	= $params['text07'][$key]; // BASIS_TM_SUPIR
				$rows[$key]['JUMLAH_TM']	= $params['text08'][$key]; // JUMLAH_TM
				$rows[$key]['SELISIH_TM']	= $params['text09'][$key]; // SELISIH_TM
				$rows[$key]['TARIF_TM']        = $params['text10'][$key]; // TARIF_TM
				$rows[$key]['RP_HK_TM']        = $params['text11'][$key]; // RP_HK_TM
				$rows[$key]['RP_BASIS_TM']        = $params['text12'][$key]; // RP_BASIS_TM
				$rows[$key]['RP_KG_BASIS_TM']        = $params['text13'][$key]; // RP_KG_BASIS_TM
				$rows[$key]['RP_PREMI_TM']        = $params['text14'][$key]; // RP_PREMI_TM
				$rows[$key]['RP_KG_PREMI_TM']        = $params['text15'][$key]; // RP_KG_PREMI_TM
				$rows[$key]['TARIF_SUPIR']        = $params['text16'][$key]; // TARIF_SUPIR
				$rows[$key]['RP_PREMI_SUPIR']        = $params['text17'][$key]; // RP_PREMI_SUPIR
				$rows[$key]['RP_KG_PREMI_SUPIR']        = $params['text18'][$key]; // RP_KG_PREMI_SUPIR
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {		
			...
			// ************************************************ SAVE NORMA PANEN LOADING TEMP ************************************************
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $uniq_code_file.'_00_TNPANENLOAD_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_saveTemp';
		...
		// ************************************************ SAVE ALL NORMA PANEN LOADING ************************************************
		//generate filename untuk .sh dan .sql
		$urutan++;
		$filename = $uniq_code_file.'_00_TNPANENLOAD_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_save';
		$this->_global->createBashFile($filename); //create bash file
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {		
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENLOAD_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanen';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENLOAD_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenTotalCost';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENLOAD_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenSaveVRA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PANEN_LOADING";
