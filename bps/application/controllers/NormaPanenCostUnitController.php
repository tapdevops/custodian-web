...
Versi				: 	1.0.0
Deskripsi			: 	Controller Class untuk Norma Panen Cost Unit
Function 			:	- listAction		: menampilkan list norma Panen Cost Unit
...
=========================================================================================================================
*/
class NormaPanenCostUnitController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPanenCostUnit();
    ...
    {
        $this->_redirect('/norma-panen-cost-unit/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Panen Cost Unit';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['JARAK_ANGKUT']	= $params['text04'][$key]; // JARAK_ANGKUT
				$rows[$key]['TARGET']	= $params['text05'][$key]; // TARGET
				$rows[$key]['RIT']	= $params['text06'][$key]; // RIT
				$rows[$key]['RP_KM_INTERNAL']	= $params['text07'][$key]; // RP_KM_INTERNAL
				$rows[$key]['RP_KG_INTERNAL']	= $params['text08'][$key]; // RP_KG_INTERNAL
				$rows[$key]['RP_KM_EXTERNAL']	= $params['text09'][$key]; // RP_KM_EXTERNAL
				$rows[$key]['RP_KG_EXTERNAL']        = $params['text10'][$key]; // RP_KG_EXTERNAL
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {		
			...
			// ************************************************ SAVE NORMA PANEN COST UNIT TEMP ************************************************
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $uniq_code_file.'_00_TNPANENCOUN_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_saveTemp';
		...
		// ************************************************ SAVE ALL NORMA PANEN COST UNIT ************************************************
		//generate filename untuk .sh dan .sql
		$urutan++;
		$filename = $uniq_code_file.'_00_TNPANENCOUN_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_save';
		$this->_global->createBashFile($filename); //create bash file
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {		
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENCOUN_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanen';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENCOUN_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenSaveVRA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PANEN_PREMI_COST_UNIT";
