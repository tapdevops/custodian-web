...
=========================================================================================================================
*/
class HoCapexController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_HoCapex();
    ...
    {
        $this->view->title = 'Budgeting HO &raquo; CAPEX HO';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
        	...
        	if ($params['tChange'][$key]) {
				...
				$rows[$key]['CAPEX_DESCRIPTION'] = $params['text07'][$key];
				...
				$rows[$key]['CAPEX_JAN'] 		= $this->parse_number($params['text15'][$key]);
				$rows[$key]['CAPEX_FEB'] 		= $this->parse_number($params['text16'][$key]);
				$rows[$key]['CAPEX_MAR'] 		= $this->parse_number($params['text17'][$key]);
				$rows[$key]['CAPEX_APR'] 		= $this->parse_number($params['text18'][$key]);
				$rows[$key]['CAPEX_MAY'] 		= $this->parse_number($params['text19'][$key]);
				$rows[$key]['CAPEX_JUN'] 		= $this->parse_number($params['text20'][$key]);
				$rows[$key]['CAPEX_JUL'] 		= $this->parse_number($params['text21'][$key]);
				$rows[$key]['CAPEX_AUG'] 		= $this->parse_number($params['text22'][$key]);
				$rows[$key]['CAPEX_SEP'] 		= $this->parse_number($params['text23'][$key]);
				$rows[$key]['CAPEX_OCT'] 		= $this->parse_number($params['text24'][$key]);
				$rows[$key]['CAPEX_NOV'] 		= $this->parse_number($params['text25'][$key]);
				$rows[$key]['CAPEX_DEC'] 		= $this->parse_number($params['text26'][$key]);
				$rows[$key]['CAPEX_TOTAL'] 		= $this->parse_number($params['text27'][$key]);
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {
			...
			// generate filename untuk .sh dan .sql
			$filename = $uniq_code_file.'_00_HO_CAPEX_01_SAVETEMP';
	...
	public function deleteAction()
    {
		...
		//generate filename untuk .sh dan .sql
		$filename = $uniq_code_file.'_00_HCPX_01_DELETE';
		$this->_global->createBashFile($filename); //create bash file			
		$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
