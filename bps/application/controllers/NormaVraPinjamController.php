...
Versi				: 	2.0.0
Deskripsi			: 	Controller Class untuk Norma VRA yang Dapat Dipinjam
...
=========================================================================================================================
*/
class NormaVraPinjamController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaVraPinjam();
    ...
    {
        $this->_redirect('/norma-vra-pinjam/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma VRA (Pinjam)';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
           if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['REGION_CODE']     	= $params['text03'][$key]; // REGION_CODE
				$rows[$key]['VRA_CODE']        	= $params['text04'][$key]; // VRA_CODE
				$rows[$key]['RP_QTY']    		= $params['text06'][$key]; // RP_QTY
            }
        }
		return $rows;
	}
	...
	public function updateInheritModule($row = array()) {
		...
		if (!empty($row)) {	
			...
			$par_opex_vra['PERIOD_BUDGET'] = $row['PERIOD_BUDGET'];
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_VRA_PINJAM";
