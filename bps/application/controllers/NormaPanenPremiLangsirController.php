...
Versi				: 	1.0.0
Deskripsi			: 	Controller Class untuk Norma Panen Premi Langsir
Function 			:	- listAction		: menampilkan list norma Panen Premi Langsir
...
=========================================================================================================================
*/
class NormaPanenPremiLangsirController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_NormaPanenPremiLangsir();
    ...
    {
        $this->_redirect('/norma-panen-premi-langsir/main');
    }

    public function mainAction()
    {
        $this->view->title = 'Data &raquo; Norma Panen Premi Langsir';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['tChange'][$key])) {
				...
				$rows[$key]['VRA_TYPE']			= $params['text05'][$key]; // VRA_TYPE
				$rows[$key]['TON_TRIP']			= $params['text06'][$key]; // TON_TRIP
				$rows[$key]['TRIP_HARI']		= $params['text07'][$key]; // TRIP_HARI
				$rows[$key]['TON_HARI']			= $params['text08'][$key]; // TON_HARI
				$rows[$key]['HM_TRIP']			= $params['text09'][$key]; // HM_TRIP
				$rows[$key]['RP_HM']       	 	= $params['text10'][$key]; // RP_HM
				$rows[$key]['RP_TRIP']        	= $params['text11'][$key]; // RP_TRIP
				$rows[$key]['RP_KG']        	= $params['text12'][$key]; // RP_KG
	...
	public function saveAction()
    {
		...
		if (!empty($rows)) {		
			...
			// ************************************************ SAVE NORMA PREMI LANGSIR TEMP ************************************************
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $uniq_code_file.'_00_TNPANENPRLA_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_saveTemp';
		...
		// ************************************************ SAVE ALL NORMA PREMI LANGSIR ************************************************
		//generate filename untuk .sh dan .sql
		$urutan++;
		$filename = $uniq_code_file.'_00_TNPANENPRLA_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'_save';
		$this->_global->createBashFile($filename); //create bash file
		...
		shell_exec("mv ".getcwd()."/tmp_query/".$filename.".sql ".getcwd()."/logs/".date("Y-m-d")."/".$filename.".sql");
		// ************************************************ SAVE ALL NORMA PANEN PREMI LANGSIR ************************************************
	...
	public function updateInheritModule($row = array()) {
		if (!empty($row)) {		
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRLA_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanen';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRLA_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenTotalCost';
			$this->_global->createBashFile($filename); //create bash file			
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
			...
			//generate filename untuk .sh dan .sql
			$urutan++;
			$filename = $row['uniq_code_file'].'_TNPANENPRLA_'.str_pad($urutan,3,"0",STR_PAD_LEFT).'RktPanenSaveVRA';
			$this->_global->createBashFile($filename); //create bash file		
			$this->_global->createSqlFile($filename, "START : ".date("Y-m-d H:i:s")."\n"); //start create file
	...
	public function saveTempAction()
    {
		...
		if (!empty($rows)) {		
			// ************************************************ SAVE NORMA PANEN PREMI LANGSIR TEMP ************************************************
			//generate filename untuk .sh dan .sql
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TN_PANEN_PREMI_LANGSIR";
