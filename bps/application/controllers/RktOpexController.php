...
Versi				: 	2.0.0
Deskripsi			: 	Controller Class untuk RKT OPEX
Function 			:	- getStatusPeriodeAction		: BDJ 22/07/2013	: cek status periode budget yang dipilih
						- listAction					: SID 10/07/2013	: menampilkan list RKT OPEX
						- mappingAction					: SID 10/07/2013	: mapping textfield name terhadap field name di DB
						- saveTempAction				: SID 10/07/2013	: simpan data sementara sesuai input user
						- saveAction					: SID 10/07/2013	: save data
						- deleteAction					: SID 10/07/2013	: hapus data
...
=========================================================================================================================
*/
class RktOpexController extends Zend_Controller_Action
{
    ...
    public function init()
    {
        ...
        $this->_model = new Application_Model_RktOpex();
    ...
    {
        $this->view->title = 'Budgeting Tahap 2 &raquo; RKT OPEX';
	...
	public function mappingAction(){
        ...
        foreach ($params['text00'] as $key => $val) {
            if (($key > 0) && ($params['text03'][$key])){
				...
				$rows[$key]['TRX_CODE']        		= $params['trxcode'][$key]; // KODE TRANSAKSI
				...
				$rows[$key]['OLD_GROUP_BUM_CODE']  	= $params['text006'][$key]; // OLD_GROUP_BUM_CODE
            }
        }
		return $rows;
	}
	...
	public function updLockedSeqStatusAction(){
        ...
        $params = $this->_request->getPost();
		$params['task_name'] = "TR_RKT_OPEX_NON_VRA";
