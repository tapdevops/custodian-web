  ...
  $.fn.freezeTableColumns = function(options) {
    options = initOptions(options);
		...
		main_div.append(
		  ...
		  '<div id="' + main_id + '-region1" style="display: inline-block; vertical-align: top; width: 5px; overflow: hidden; border-right: 3px solid red;"><div></div></div>' + 
      ...
      '<div id="' + main_id + '-row2"  style="white-space: nowrap;">' +
		  '<div id="' + main_id + '-region3" style="display: inline-block; vertical-align: top; height: 100%; width: 5px; overflow-y: hidden; overflow-x: scroll; border-right: 3px solid red;"><div></div></div>' + 
    ...
    $('#' + main_id + '-region3').children('div').children('table').append('<tbody id="data_freeze"></tbody>');
    var tbody = $('#' + main_id + '-region3 > div > table > tbody');
    source_table.children('tbody').children('tr').each(function(rowindex, rowelement) {
	 //doni
	  if (rowindex == 0) tbody.append('<tr style=\'display:none\'></tr>');
      else 
	  tbody.append('<tr></tr>');
  ...
  $.fn.freezeTableColumnsLayout = function(options) {
    ...
    var main_id = $(this).attr('id');
	    ...
	    bottomRegion.children('div').children('table').prepend('<colgroup></colgroup>');
		  ...
		  //width text box
  		  var textwidth = Math.max($(this).width(), bottomcell.width());
		  bottomRegion.children('div').children('table').children('tbody').children('tr').children('td').children('input[type=text], input[type=button], select').eq(index).css('width', textwidth);
  	  });//each
    ...
    var region2_4div = Math.max($('#' + main_id + '-region2').children('div').children('table').outerWidth(), $('#' + main_id + '-region4').children('div').children('table').outerWidth());
