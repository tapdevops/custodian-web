...
if(isset($_GET["act"]))
{
	...
	if($act=='add') {
		...
		<tr>
			<td width='30'>Nama</td>
			<td width='70%'>
				<input id='txtTHROLD_UserID' type='hidden' value='$mv_UserID'/>
				$field[FullName]
			</td>
		</tr>
		<tr>
...
elseif(isset($_POST[addheader])) {
	...
	if($mysqli->query($sql)) {
		...
		$sql1= "INSERT INTO TH_ReleaseOfLegalDocument
				VALUES (NULL,'$CT_Code',sysdate(),'$mv_UserID','$_POST[txtTHROLD_THLOLD_Code]',
	...
	for ($i=0 ; $i<$sum ; $i++) {
		...
		$sql1= "INSERT INTO TD_ReleaseOfLegalDocument
				VALUES (NULL,NULL,'$_POST[txtTDROLD_THROLD_ID]', '$TDLOLD_ID[$i]','$txtTDROLD_Information[$i]',
						'$TDROLD_LeadTime',NULL,'$mv_UserID', sysdate(),'$mv_UserID',
						sysdate(),NULL,NULL)";
		$sql2= "UPDATE TD_LoanOfLegalDocument
				SET TDLOLD_Response='1', TDLOLD_Update_UserID='$mv_UserID',TDLOLD_Update_Time=sysdate()
