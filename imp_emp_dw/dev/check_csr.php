<!--
=========================================================================================================================
Project				: 	Sinkronisasi Table MASTER dengan table di aplikasi CSR
Versi				: 	1.0.0
Deskripsi			: 	Sinkronisasi user table master dengan user di aplikasi CSR
...
while($db->nextRecord()) {
	...
	//insert data yang tidak terdapat di db_csr.M_Role
	if ($emp_nik=="") {
		echo "## Insert Data : ".$db->Record['Employee_NIK']." | ".$db->Record['Employee_FullName']."<br>";
		
		$sqlqry = "INSERT INTO db_csr.M_Role (	Role_UserNIK, 
										Role_Code,
										Role_Status,
										Role_InsertUser,
										Role_InsertTime,
	...
	//update data yang telah terdapat di M_Employee
		...
		$sqlqry = "UPDATE db_csr.M_Role 
				   SET Role_UpdateUser = 'auto_syn_hris',
