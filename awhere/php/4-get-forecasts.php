...
if($forecastStatusCode==200){  	// Code 200 means the request was successful
	...
	echo "<table><tr>
				<td>date</td>
				<td>conditionsCode</td>
				<td>conditionsText</td>
				<td colspan='2'>temperatures</td>
				<td colspan='2'>precipitation</td>
				<td colspan='2'>sky</td>
			</tr>";	
	foreach($forecastResponse->forecasts as $index=>$data){
		echo "<tr><td>".$data->date."</td>";
				foreach($data->forecast as $index => $datax) {
				  echo"<td>".$datax->conditionsCode."</td>
						<td>".$datax->conditionsText."</td>
						<td>".$datax->temperatures->max."|".$datax->temperatures->min."</td>";
				}
			echo "</tr>";
	}
	echo "</table>";
