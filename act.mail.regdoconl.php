			...
			if ($A_Status=='3') {
				...
				if ($step <> $jStep) {
					...
					$query_get_company = "SELECT DISTINCT
							...
							END AS company_name, td.TDROONLD_Core_CompanyID AS td_core_companyid
						...
						WHERE td.TDROONLD_THROONLD_ID='$h_arr[THROONLD_ID]' AND td.TDROONLD_Delete_Time IS NULL";
					...
					while($arr_gc = mysql_fetch_array($sql_gc)){
						...
						if( $arr_gc['td_core_companyid'] == null ){
							...
							$query_jumlah_row = "SELECT * 
								FROM TD_RegistrationOfOtherNonLegalDocuments
								WHERE TDROONLD_THROONLD_ID='$h_arr[THROONLD_ID]'
								AND TDROONLD_Core_CompanyID='$company_id'
								AND TDROONLD_Delete_Time IS NULL";
							$jumlahRow = mysql_num_rows(mysql_query($query_jumlah_row));
						}
						...
						while($arr_gc = mysql_fetch_array($sql_gc)){
							...
							if( $arr_gc['td_core_companyid'] == null ){
								...
								$d_query="SELECT *
									  ...
									  WHERE TDROONLD_THROONLD_ID='$h_arr[THROONLD_ID]'
									  AND TDROONLD_Delete_Time IS NULL
									  AND TDROONLD_Core_CompanyID='$Core_CompanyID'";
