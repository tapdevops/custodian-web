...
if(isset($_GET["act"])) {
	...
	elseif($act=='adddetail')	{
		...
		<tr>
			<td>Grup Dokumen</td>
			<td>
				<input id='txtTHLOLD_DocumentGroupID' type='hidden' value='$DocumentGroup_ID'/>
...
elseif(isset($_POST[adddetail])) {
	...
	for ($i=1 ; $i<=$count ; $i++) {
		...
		$sql1= "INSERT INTO TD_LoanOfLegalDocument 
				VALUES (NULL,NULL,'$_POST[txtTDLOLD_THLOLD_ID]', '$optTDLOLD_DocumentCategoryID', 
						'$txtTDLOLD_DocumentCode','$txtTDLOLD_Information', '0','$_SESSION[User_ID]', 
		...
		$sql5= "UPDATE M_DocumentLegal 
			    SET DL_Status ='$docStatus',DL_Update_UserID='$_SESSION[User_ID]',DL_Update_Time=sysdate()
	...
	$sql4= "UPDATE TH_LoanOfLegalDocument 
			...
			THLOLD_Update_UserID='$_SESSION[User_ID]',THLOLD_Update_Time=sysdate()
			WHERE THLOLD_LoanCode='$_POST[txtTDLOLD_THLOLD_LoanCode]'
